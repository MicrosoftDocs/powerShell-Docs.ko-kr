...
트리거 조건이 있는.NET 형식의 집합을 지정 합니다. 이 형식의 값이 있는 경우에 조건이 충족 됩니다.

구성 요소 DefaultSettings (형식) EnumerableExpansions 요소 (형식) EnumerableExpansions 요소 (형식) EntrySelectedBy 요소에 대 한 EntrySelectedBy SelectionCondition 요소 EnumerableExpansion (형식)에 EntrySelectedBy EnumerableExpansion (형식)에 대 한에 대 한 SelectionCondition SelectionSetName 요소 EnumerableExpansion (형식)
