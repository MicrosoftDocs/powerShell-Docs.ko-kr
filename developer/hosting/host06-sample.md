...
이 샘플에서는 명령줄에서 명령을 읽고 명령을 실행 하 고 다음 콘솔에 결과 표시 하는 대화형 콘솔 기반 호스트 응용 프로그램을 빌드하는 방법을 보여 줍니다. 또한 이 샘플에서는 토크나이저 API를 사용하여 사용자가 입력하는 텍스트의 색을 지정합니다.
...
- 빌드하는 대화형 Windows PowerShell 셸을 이러한 호스트 클래스를 사용 하는 콘솔 응용 프로그램을 작성 합니다.

- 만들기는 `$profile` 변수와 다음 프로필을 로드 합니다.
...
- 토큰화 API를 사용 하 여 색상은 입력 한 대로 명령줄을 지정 합니다.

## <a name="example"></a>예제

 이 예제에서는 명령줄에서 명령을 입력 하면, 이러한 명령을 처리 및 다음 결과 출력 합니다.
...
## <a name="example"></a>예제
 ...
 다음 코드는 구현의 합니다 [System.Management.Automation.Host.PSHost](/dotnet/api/System.Management.Automation.Host.PSHost) 이 호스트 응용 프로그램에서 사용 되는 클래스입니다. 구현 되지 않은 요소 예외를 throw 하거나 아무 것도 반환 합니다.
...
## <a name="example"></a>예제
 ...
 다음 코드는 구현의 합니다 [System.Management.Automation.Host.Pshostrawuserinterface](/dotnet/api/System.Management.Automation.Host.PSHostRawUserInterface) 이 호스트 응용 프로그램에서 사용 되는 클래스입니다. 구현 되지 않은 요소 예외를 throw 하거나 아무 것도 반환 합니다.
...
## <a name="example"></a>예제
 ...
 다음 코드를 읽고 명령줄 및 색을 텍스트를 입력할 때. 토큰 사용 하 여 결정 되는 [System.Management.Automation.Psparser.Tokenize*](/dotnet/api/System.Management.Automation.PSParser.Tokenize) 메서드.
