...
정렬 된 개체는 파이프라인에서로 전송 됩니다 `Format-Table` . **View** 매개 변수는 PowerShell 파일에서 system.object에 대해 정의 된 **StartTime** 뷰를 지정 합니다. `DotNetTypes.format.ps1xml`  **StartTime** 보기는 각 프로세스 시작 시간을 간단한 날짜로 변환한 다음 시작 날짜별로 프로세스를 그룹화 합니다.
...
이 예제에서 사용자 지정 보기에는 디렉터리의 내용이 표시 됩니다. 사용자 지정 뷰는에서 만든 **system.io.directoryinfo** 및 CreationTime 개체에 **대 한 테이블** 출력에  열을 추가 합니다 `Get-ChildItem` .
