...
에서 만든 모듈의 선택적 경로와 이름을 지정 합니다 `Export-PSSession` . 기본 경로는 `$home\Documents\WindowsPowerShell\Modules`입니다. 이 매개 변수는 필수입니다.
...
명령을 내보낼 원본 PSSession을 지정합니다. 세션 개체 또는 세션 개체를 가져오는 명령 (예: 명령)을 포함 하는 변수를 입력 `Get-PSSession` 합니다. 이 매개 변수는 필수입니다.
