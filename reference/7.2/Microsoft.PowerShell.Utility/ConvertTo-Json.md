...
PowerShell 7.2을 통해 `ConvertTo-Json` 입력 개체의 깊이가 명령에 지정 된 깊이를 초과 하는 경우에서 경고를 내보냅니다. 이렇게 하면 개체를 변환할 때 원치 않는 데이터 손실이 방지 됩니다.
...
JSON 표시에 포함되는 포함 개체의 수준을 지정합니다. 기본값은 2입니다. PowerShell 7.2에서 `ConvertTo-Json` 입력 개체의 수준 수가이 값을 초과 하는 경우에서 경고를 내보냅니다.
