...
PowerShell 6에서 `Sort-Object` **아래쪽** 매개 변수는 대신 사용할 수 `Select-Object` 있습니다. 예를 들어 `Get-Process | Sort-Object -Property WS -Bottom 5`입니다.
...
**Status** 는 열거 된 속성입니다. **중지 됨** 의 값은 **1** 이 고 **실행** 값은 **4** 입니다. 중지  된 `$True` 프로세스 전에 **실행 중인** 프로세스가 표시 되도록 내림차순 매개 변수가로  설정 됩니다. **DisplayName** 은 **내림차순** 매개 변수를로 설정 `$False` 하 여 표시 이름을 알파벳 순서로 정렬 합니다.
