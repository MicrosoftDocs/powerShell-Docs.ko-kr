...
## 설명
RESOURCE_URI로 지정 된 개체에 대 **한 작업을 실행 합니다** . 여기서 매개 변수는 키 값 쌍으로 지정 됩니다.
...
이 명령은 스풀러 서비스에 해당 하는 Win32_Service WMI 클래스 인스턴스의 **StartService** 메서드를 호출 합니다.
...
이 명령은 파일의 입력을 사용 하 여 스풀러 서비스에 대해 **StopService** 메서드를 호출 합니다.
Input.xml 파일은 다음 콘텐츠를 포함합니다.

`<p:StopService_INPUT xmlns:p="http://schemas.microsoft.com/wbem/wsman/1/wmi/root/cimv2/Win32_Service" />`

### 예제 3: 지정 된 매개 변수 값을 사용 하 여 메서드 호출
...
이 명령은 Win32_Process 클래스의 **Create** 메서드를 호출 합니다.
이 메서드는 두 개의 매개 변수 값 Notepad.exe와 C:\.를 전달 합니다.
그 결과 메모장을 실행 하는 새 프로세스가 만들어지고 새 프로세스의 현재 디렉터리는 C:\.로 설정 됩니다.

### 예제 4: 원격 컴퓨터에서 메서드 호출
...
이 명령은 스풀러 서비스에 해당 하는 Win32_Service WMI 클래스 인스턴스의 **StartService** 메서드를 호출 합니다.
*ComputerName* 매개 변수가 지정 되었으므로 명령은 원격 server01 컴퓨터에 대해 실행 됩니다.
...
*ResourceURI* 매개 변수 및 *SelectorSet* 매개 변수를 사용 하 여 관리 리소스를 지정 합니다.
예를 들어 다음 명령은 *FilePath* 매개 변수를 사용 합니다.
...
*ResourceURI* 및 *SelectorSet* 를 사용 하 여 관리 리소스를 지정 합니다.
*Valueset* 매개 변수 값은 해시 테이블 이어야 합니다.
