...
작업의 출력을 보려면 cmdlet을 사용 `Receive-Job` 합니다. 예를 들어 `Receive-Job -Id 1`입니다.
...
작업의 출력을 보려면 cmdlet을 사용 `Receive-Job` 합니다. 예를 들어 `Receive-Job -Id 5`입니다.
...
체제가 있어도 runas32를 사용 하 여 32 비트 PowerShell ( `pwsh` ) 프로세스 를 시작 하려면 32 비트 powershell이 설치 되어 있어야 합니다.
...
백그라운드 작업에서 실행할 명령을 지정합니다. 스크립트 블록을 만들려면 명령을 중괄호 ()로 묶습니다 `{}` . `$input`자동 변수를 사용 하 여 **InputObject** 매개 변수의 값에 액세스 합니다. 이 매개 변수는 필수입니다.
