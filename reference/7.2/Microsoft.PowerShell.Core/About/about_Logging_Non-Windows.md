...
일반적으로 PowerShell 로그 항목은 syslog의 기본값에 기록 됩니다 `location/file` .  그러나 항목을 사용자 지정 파일로 리디렉션할 수 있습니다.
...
1.  `pwsh` PowerShell 주 이진에 대해 검색 필드에를 입력 합니다.
...
|`LogLevel`   |항상        |단일 값을 지정 합니다. 이 값을 사용 하면  |
