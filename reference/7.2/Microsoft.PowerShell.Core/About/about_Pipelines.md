...
그러나의 **InputObject** 매개 변수를 사용 하는 경우은 (는) `Get-Member` `Get-Member` 단일 단위로 **시스템 진단** 개체의 배열을 받습니다. 개체의 배열 속성을 표시 합니다. (System.string 형식 이름 뒤의 배열 기호 ()를 적어둡니다 `[]` .) 
...
> [!IMPORTANT]
> 셸에서 대화형으로 작업 하는 경우 <kbd>Ctrl</kbd>V를 사용 하 여 붙여 넣는 경우에만 줄의 시작 부분에 파이프라인을 사용 하 여 코드를 붙여 + <kbd></kbd> 넣습니다.
