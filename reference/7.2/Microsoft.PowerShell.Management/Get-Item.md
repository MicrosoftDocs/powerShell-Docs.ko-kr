...
파일에서 지정한 대체 데이터 스트림을 가져옵니다. 스트림 이름을 입력합니다. 와일드카드가 지원됩니다. 모든 스트림을 가져오려면 별표 ()를 사용 `*` 합니다. 이 매개 변수는 디렉터리에 사용할 수 있지만 기본적으로 디렉터리에는 데이터 스트림이 없습니다.

이 매개 변수는 PowerShell 3.0에서 도입 되었습니다.  PowerShell 7.2 `Get-Item` 부터는 디렉터리 및 파일에서 대체 데이터 스트림을 가져올 수 있습니다.
