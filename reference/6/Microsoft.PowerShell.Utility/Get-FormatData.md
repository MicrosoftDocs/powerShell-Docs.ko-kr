...
> [!IMPORTANT]
> 전체 형식 형식 지정 정보가 반환 되도록 하려면 **PowerShellVersion** `5.1` 의 로컬 호출을 사용할 때 항상 값과 함께 PowerShellVersion 매개 변수를 포함 해야 합니다 `Get-FormatData` . 매개 변수와 값을 생략 하면 올바른 형식 정보를 모두 가져올 수 없습니다.
