...
`New-ModuleManifest` 다음과 같이 인코드된 모듈 매니페스트 ( `.psd1` ) 파일을 만듭니다.

- Windows에서 실행 되는 PowerShell 6.0은 **UTF16** 을 사용 합니다.
- Windows가 아닌 플랫폼에서 실행 되는 PowerShell 6.0은 **UTF8NoBOM** 를 사용 합니다.
- Windows 및 비 Windows 플랫폼에서 실행 되는 PowerShell 6.1 이상에서는 **UTF8NoBOM** 를 사용 합니다.
