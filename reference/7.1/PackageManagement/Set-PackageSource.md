...
는 `Set-PackageSource` 지정 된 패키지 공급자에 대 한 패키지 소스를 바꿉니다. 패키지 원본은 항상 패키지 공급자에 의해 관리 됩니다.

## 예제

### 예제 1: 패키지 원본 변경

이 명령은 패키지 원본의 기존 이름을 변경 합니다. 원본은 **신뢰할** 수 있는 것으로 설정 되어 패키지가 설치 될 때 원본을 확인 하는 메시지를 표시 하지 않습니다.
...
변경 하려는 패키지를 나타내는 패키지 원본 ID 개체를 지정 합니다. 패키지 원본 Id는 cmdlet의 결과에 포함 됩니다 `Get-PackageSource` .
...
현재 패키지 원본 위치를 지정 합니다. 값은 URI, 파일 경로 또는 패키지 공급자가 지 원하는 다른 모든 대상 형식일 수 있습니다.
...
패키지 원본에 대 한 새 위치를 지정 합니다. 값은 URI, 파일 경로 또는 패키지 공급자가 지 원하는 다른 모든 대상 형식일 수 있습니다.
...
소스가 신뢰할 수 있는 패키지 공급자 임을 나타냅니다. 신뢰할 수 있는 원본에서 패키지 설치를 확인 하는 메시지가 표시 되지 않습니다.
...
### `Set-PackageSource` 파이프라인 입력을 허용 하지 않습니다.
