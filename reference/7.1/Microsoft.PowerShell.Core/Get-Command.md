...
이 예제에서는 별칭의 표준 이름과 함께 구문을 가져오는 방법을 보여 줍니다.

명령의 출력에는 표준 이름과 구문을 차례로 표시 하는 레이블이 지정 된 별칭이 표시 됩니다.
...
### 예 12: 메모장 명령의 모든 인스턴스 가져오기

이 예에서는 cmdlet의 **all** 매개 변수 `Get-Command` 를 사용 하 여 `Notepad` 로컬 컴퓨터에 있는 명령의 모든 인스턴스를 표시 합니다.
...
### 예제 13: cmdlet이 포함 된 모듈의 이름 가져오기
...
### 예제 14: 출력 형식이 있는 cmdlet 및 함수 가져오기
...
명령의 첫 번째 부분에서 모든 cmdlet을 가져옵니다. 파이프라인 연산자 ()는 cmdlet에 cmdlet을 보냅니다 .이 cmdlet은 `|` `Where-Object` **OutputType** 속성이 채워지는 항목만 선택 합니다. 다른 파이프라인 연산자는 선택한 cmdlet 개체를 cmdlet으로 전송 합니다 `Format-List` . 그러면이 cmdlet에서 각 cmdlet의 이름과 출력 유형을 목록으로 표시 합니다.

cmdlet 코드에서 cmdlet의 **OutputType** 특성을 정의하는 경우에만 **CommandInfo** 개체의 **OutputType** 속성에 null이 아닌 값이 포함됩니다.

### 예 15: 특정 개체 유형을 입력으로 사용 하는 cmdlet 가져오기
...
### 예제 16: 유사 항목 일치를 사용 하 여 명령 가져오기
...
명령에서 문자 일치를 사용 하 여 명령에서 대문자를 찾는 것을 나타냅니다. 예를 들어는 `i-psdf` `Import-PowerShellDataFile` 찾으려는 각 문자가 결과에서 대문자와 일치 하는 것과 일치 합니다. 이 유형의 일치를 사용 하는 경우 와일드 카드를 사용 하면 일치 항목이 발생 하지 않습니다.
