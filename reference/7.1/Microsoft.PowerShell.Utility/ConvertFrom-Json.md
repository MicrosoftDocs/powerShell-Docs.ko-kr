...
이 명령은 `-NoEnumerate` 스위치를 사용 하 여 단일 요소 JSON 배열을 라운드트립 하는 예제를 보여 줍니다.
...
JSON 문자열은 요소가 하나인 배열을 포함 합니다. 스위치가 없으면 JSON을 PSObject로 변환한 다음 명령으로 다시 변환 하 여 `ConvertTo-Json` 단일 정수를 반환 합니다.
...
출력이 열거 되지 않도록 지정 합니다.

이 매개 변수를 설정 하면 모든 요소를 개별적으로 전송 하는 대신 배열이 단일 개체로 전송 됩니다. 이를 통해 JSON은를 통해 라운드트립 될 수 있습니다 `ConvertTo-Json` .
