...
PowerShell 6.0부터는 `Import-Csv` 이제 W3C 확장 로그 파일 형식을 지원 합니다.
...
가져온 CSV 파일의 인코딩을 지정 합니다. 기본값은 `utf8NoBOM`입니다.
...
`Import-Csv` 는 W3C 확장 로그 형식도 지원 합니다. 로 시작 하는 줄은 주석이로 시작 하 고 `#` `#Fields:` 구분 기호로 분리 된 열 이름 목록을 포함 하지 않는 한 주석으로 처리 되 고 무시 됩니다. 이 경우 cmdlet은 이러한 열 이름을 사용 합니다. Windows IIS 및 기타 웹 서버 로그에 대 한 표준 형식입니다. 자세한 내용은 [확장 로그 파일 형식](https://www.w3.org/TR/WD-logfile.html)을 참조 하세요.
