...
### <span data-ttu-id="20e9e-108">StandardMethodNoProxy</span><span class="sxs-lookup"><span data-stu-id="20e9e-108">StandardMethodNoProxy</span></span>
...
## <span data-ttu-id="20e9e-111">Description</span><span class="sxs-lookup"><span data-stu-id="20e9e-111">Description</span></span>
...
<span data-ttu-id="20e9e-145">키에 목록을 제공 하면 `hobbies` `hobbies` 필드는 각 목록 항목에 대해 한 번씩 제출에 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-145">By supplying a list to the `hobbies` key, the `hobbies` field will be present in the submissions once for each list item.</span></span>
...
<span data-ttu-id="20e9e-174">이 기능은 PowerShell 6.0.0에서 추가 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-174">This feature was added in PowerShell 6.0.0.</span></span>
...
<span data-ttu-id="20e9e-197">인증서 지문을 가져오려면 `Get-Item` `Get-ChildItem` PowerShell 드라이브에서 또는 명령을 사용 `Cert:` 합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-197">To get a certificate thumbprint, use the `Get-Item` or `Get-ChildItem` command in the PowerShell `Cert:` drive.</span></span>
...
<span data-ttu-id="20e9e-217">예제:</span><span class="sxs-lookup"><span data-stu-id="20e9e-217">Example:</span></span>
...
<span data-ttu-id="20e9e-229">이 스위치를 사용 하는 경우 cmdlet은 결과 페이지의 컬렉션을 반환 합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-229">When using this switch, the cmdlet returns a collection of pages of results.</span></span> <span data-ttu-id="20e9e-230">결과의 각 페이지에는 여러 결과 항목이 포함 될 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-230">Each page of results may contain multiple result items.</span></span>
...
### <span data-ttu-id="20e9e-256">-InFile</span><span class="sxs-lookup"><span data-stu-id="20e9e-256">-InFile</span></span>
...
- <span data-ttu-id="20e9e-282">게시</span><span class="sxs-lookup"><span data-stu-id="20e9e-282">Post</span></span>
...
<span data-ttu-id="20e9e-302">Cmdlet이 `Authorization` 리디렉션 전체에 헤더 (있는 경우)를 유지 해야 함을 나타냅니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-302">Indicates the cmdlet should preserve the `Authorization` header, when present, across redirections.</span></span>
...
<span data-ttu-id="20e9e-311">**Proxy** 매개 변수에 지정된 프록시 서버를 사용할 권한이 있는 사용자 계정을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-311">Specifies a user account that has permission to use the proxy server that is specified by the **Proxy** parameter.</span></span> <span data-ttu-id="20e9e-312">기본값은 현재 사용자입니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-312">The default is the current user.</span></span>
...
### <span data-ttu-id="20e9e-320">-ResponseHeadersVariable</span><span class="sxs-lookup"><span data-stu-id="20e9e-320">-ResponseHeadersVariable</span></span>
...
<span data-ttu-id="20e9e-332">원격 서버에서 다운로드 다시 시작을 지원 하지 않는 경우 로컬 파일을 덮어쓰고 전체 원격 파일이 완전히 다시 다운로드 됩니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-332">If the remote server does not support download resuming, then the local file will be overwritten and the entire remote file will be completely re-downloaded.</span></span> <span data-ttu-id="20e9e-333">이 동작은 **다시 시작** 없이 **출력** 을 사용 하는 것과 같습니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-333">This behavior is the same as using **OutFile** without **Resume**.</span></span>
...
<span data-ttu-id="20e9e-341">이 cmdlet이 웹 요청 세션을 만들고 값에 저장 하는 데 사용할 변수를 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-341">Specifies a variable for which this cmdlet creates a web request session and saves it in the value.</span></span>
...
<span data-ttu-id="20e9e-354">만료, 해지, 신뢰할 수 있는 루트 인증 기관 등의 모든 유효성 검사를 포함 하는 인증서 유효성 검사를 건너뜁니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-354">Skips certificate validation checks that include all validations such as expiration, revocation, trusted root authority, etc.</span></span>
...
### <span data-ttu-id="20e9e-366">-SkipHttpErrorCheck</span><span class="sxs-lookup"><span data-stu-id="20e9e-366">-SkipHttpErrorCheck</span></span>
...
<span data-ttu-id="20e9e-378">이 기능은 PowerShell 6.0.0에서 추가 되었으며 `Tls13` powershell 7.1에 대 한 지원이 추가 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-378">This feature was added in PowerShell 6.0.0 and support for `Tls13` was added in PowerShell 7.1.</span></span>
...
### <span data-ttu-id="20e9e-388">-Token</span><span class="sxs-lookup"><span data-stu-id="20e9e-388">-Token</span></span>

<span data-ttu-id="20e9e-389">요청에 포함할 OAuth 또는 전달자 토큰입니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-389">The OAuth or Bearer token to include in the request.</span></span> <span data-ttu-id="20e9e-390">**토큰** 은 특정 **인증** 옵션에 필요 합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-390">**Token** is required by certain **Authentication** options.</span></span> <span data-ttu-id="20e9e-391">독립적으로 사용할 수 없습니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-391">It can't be used independently.</span></span>

<span data-ttu-id="20e9e-392">**토큰** `SecureString` 은 토큰이 포함 된를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-392">**Token** takes a `SecureString` that contains the token.</span></span> <span data-ttu-id="20e9e-393">토큰을 제공 하려면 다음을 수동으로 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-393">To supply the token, manually use the following:</span></span>

`Invoke-RestMethod -Uri $uri -Authentication OAuth -Token (Read-Host -AsSecureString)`

<span data-ttu-id="20e9e-394">이 매개 변수는 PowerShell 6.0에서 도입 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-394">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="20e9e-395">-전송자 인코딩</span><span class="sxs-lookup"><span data-stu-id="20e9e-395">-TransferEncoding</span></span>

<span data-ttu-id="20e9e-396">transfer-encoding HTTP 응답 헤더의 값을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-396">Specifies a value for the transfer-encoding HTTP response header.</span></span> <span data-ttu-id="20e9e-397">이 매개 변수에 허용되는 값은 다음과 같습니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-397">The acceptable values for this parameter are:</span></span>

- <span data-ttu-id="20e9e-398">청크</span><span class="sxs-lookup"><span data-stu-id="20e9e-398">Chunked</span></span>
- <span data-ttu-id="20e9e-399">압축</span><span class="sxs-lookup"><span data-stu-id="20e9e-399">Compress</span></span>
- <span data-ttu-id="20e9e-400">Deflate</span><span class="sxs-lookup"><span data-stu-id="20e9e-400">Deflate</span></span>
- <span data-ttu-id="20e9e-401">GZip</span><span class="sxs-lookup"><span data-stu-id="20e9e-401">GZip</span></span>
- <span data-ttu-id="20e9e-402">ID</span><span class="sxs-lookup"><span data-stu-id="20e9e-402">Identity</span></span>
...
### <span data-ttu-id="20e9e-403">-Uri</span><span class="sxs-lookup"><span data-stu-id="20e9e-403">-Uri</span></span>

<span data-ttu-id="20e9e-404">웹 요청이 전송 되는 인터넷 리소스의 URI (Uniform Resource Identifier)를 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-404">Specifies the Uniform Resource Identifier (URI) of the internet resource to which the web request is sent.</span></span> <span data-ttu-id="20e9e-405">이 매개 변수는 HTTP, HTTPS, FTP 및 FILE 값을 지원합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-405">This parameter supports HTTP, HTTPS, FTP, and FILE values.</span></span>

<span data-ttu-id="20e9e-406">이 매개 변수는 필수적 요소입니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-406">This parameter is required.</span></span> <span data-ttu-id="20e9e-407">매개 변수 이름 ( **Uri** )은 선택 사항입니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-407">The parameter name ( **Uri** ) is optional.</span></span>
...
### <span data-ttu-id="20e9e-408">-UseBasicParsing 분석</span><span class="sxs-lookup"><span data-stu-id="20e9e-408">-UseBasicParsing</span></span>

<span data-ttu-id="20e9e-409">이 매개 변수는 더 이상 사용 되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-409">This parameter has been deprecated.</span></span> <span data-ttu-id="20e9e-410">PowerShell 6.0.0부터 모든 웹 요청은 기본 구문 분석만 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-410">Beginning with PowerShell 6.0.0, all Web requests use basic parsing only.</span></span> <span data-ttu-id="20e9e-411">이 매개 변수는 이전 버전과의 호환성을 위해서만 포함 되며 cmdlet의 작업에는 영향을 주지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-411">This parameter is included for backwards compatibility only and any use of it will have no effect on the operation of the cmdlet.</span></span>
...
### <span data-ttu-id="20e9e-412">-UseDefaultCredentials</span><span class="sxs-lookup"><span data-stu-id="20e9e-412">-UseDefaultCredentials</span></span>

<span data-ttu-id="20e9e-413">Cmdlet이 현재 사용자의 자격 증명을 사용 하 여 웹 요청을 보내는 것을 나타냅니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-413">Indicates that the cmdlet uses the credentials of the current user to send the web request.</span></span> <span data-ttu-id="20e9e-414">이는 **인증** 또는 **자격 증명과** 함께 사용할 수 없으며, 일부 플랫폼에서 지원 되지 않을 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-414">This can't be used with **Authentication** or **Credential** and may not be supported on all platforms.</span></span>
...
### <span data-ttu-id="20e9e-415">-UserAgent</span><span class="sxs-lookup"><span data-stu-id="20e9e-415">-UserAgent</span></span>

<span data-ttu-id="20e9e-416">웹 요청에 대한 사용자 에이전트 문자열을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-416">Specifies a user agent string for the web request.</span></span>

<span data-ttu-id="20e9e-417">기본 사용자 에이전트는 `Mozilla/5.0 (Windows NT 10.0; Microsoft Windows 10.0.15063; en-US) PowerShell/6.0.0` 각 운영 체제 및 플랫폼에 대해 약간 변형 된와 비슷합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-417">The default user agent is similar to `Mozilla/5.0 (Windows NT 10.0; Microsoft Windows 10.0.15063; en-US) PowerShell/6.0.0` with slight variations for each operating system and platform.</span></span>

<span data-ttu-id="20e9e-418">대부분의 인터넷 브라우저에서 사용 되는 표준 사용자 에이전트 문자열을 사용 하 여 웹 사이트를 테스트 하려면 Chrome, FireFox, InternetExplorer, Opera 및 Safari와 같은 [PSUserAgent](/dotnet/api/microsoft.powershell.commands.psuseragent) 클래스의 속성을 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-418">To test a website with the standard user agent string that is used by most internet browsers, use the properties of the [PSUserAgent](/dotnet/api/microsoft.powershell.commands.psuseragent) class, such as Chrome, FireFox, InternetExplorer, Opera, and Safari.</span></span>
...
### <span data-ttu-id="20e9e-419">-WebSession</span><span class="sxs-lookup"><span data-stu-id="20e9e-419">-WebSession</span></span>

<span data-ttu-id="20e9e-420">웹 요청 세션을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-420">Specifies a web request session.</span></span> <span data-ttu-id="20e9e-421">달러 기호 ()를 포함 하 여 변수 이름을 입력 `$` 합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-421">Enter the variable name, including the dollar sign (`$`).</span></span>

<span data-ttu-id="20e9e-422">웹 요청 세션의 값을 재정의하려면 **UserAgent** 또는 **Credential** 과 같은 cmdlet 매개 변수를 사용합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-422">To override a value in the web request session, use a cmdlet parameter, such as **UserAgent** or **Credential**.</span></span> <span data-ttu-id="20e9e-423">매개 변수 값이 웹 요청 세션의 값보다 우선합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-423">Parameter values take precedence over values in the web request session.</span></span> <span data-ttu-id="20e9e-424">`Content-Type` **본문** 에 대해 **MultipartFormDataContent** 개체를 제공 하는 경우와 같은 콘텐츠 관련 헤더만 재정의 됩니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-424">Content related headers, such as `Content-Type`, will be also be overridden when a **MultipartFormDataContent** object is supplied for **Body**.</span></span>

<span data-ttu-id="20e9e-425">원격 세션과 달리 웹 요청 세션은 영구 연결이 아닙니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-425">Unlike a remote session, the web request session isn't a persistent connection.</span></span> <span data-ttu-id="20e9e-426">쿠키, 자격 증명, 최대 리디렉션 값 및 사용자 에이전트 문자열을 포함 하 여 연결 및 요청에 대 한 정보를 포함 하는 개체입니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-426">It's an object that contains information about the connection and the request, including cookies, credentials, the maximum redirection value, and the user agent string.</span></span> <span data-ttu-id="20e9e-427">이 개체를 사용하여 웹 요청 간에 상태와 데이터를 공유할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-427">You can use it to share state and data among web requests.</span></span>

<span data-ttu-id="20e9e-428">웹 요청 세션을 만들려면 명령의 **Sessionvariable** 매개 변수 값에 달러 기호 없이 변수 이름을 입력 `Invoke-RestMethod` 합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-428">To create a web request session, enter a variable name, without a dollar sign, in the value of the **SessionVariable** parameter of an `Invoke-RestMethod` command.</span></span> <span data-ttu-id="20e9e-429">`Invoke-RestMethod` 세션을 만들어 변수에 저장 합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-429">`Invoke-RestMethod` creates the session and saves it in the variable.</span></span> <span data-ttu-id="20e9e-430">후속 명령에서 변수를 **WebSession** 매개 변수 값으로 사용합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-430">In subsequent commands, use the variable as the value of the **WebSession** parameter.</span></span>

<span data-ttu-id="20e9e-431">동일한 명령에서 **Sessionvariable** 및 **websession** 매개 변수를 사용할 수 없습니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-431">You can't use the **SessionVariable** and **WebSession** parameters in the same command.</span></span>
...
### <span data-ttu-id="20e9e-432">CommonParameters</span><span class="sxs-lookup"><span data-stu-id="20e9e-432">CommonParameters</span></span>

<span data-ttu-id="20e9e-433">이 cmdlet 일반 매개 변수를 지원합니다. -Debug, -ErrorAction, -ErrorVariable, -InformationAction, -InformationVariable, -OutVariable, -OutBuffer, -PipelineVariable, -Verbose, -WarningAction 및 -WarningVariable.</span><span class="sxs-lookup"><span data-stu-id="20e9e-433">This cmdlet supports the common parameters: -Debug, -ErrorAction, -ErrorVariable, -InformationAction, -InformationVariable, -OutVariable, -OutBuffer, -PipelineVariable, -Verbose, -WarningAction, and -WarningVariable.</span></span> <span data-ttu-id="20e9e-434">자세한 내용은 [about_CommonParameters](https://go.microsoft.com/fwlink/?LinkID=113216)를 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="20e9e-434">For more information, see [about_CommonParameters](https://go.microsoft.com/fwlink/?LinkID=113216).</span></span>

## <span data-ttu-id="20e9e-435">입력</span><span class="sxs-lookup"><span data-stu-id="20e9e-435">INPUTS</span></span>

### <span data-ttu-id="20e9e-436">System.Object</span><span class="sxs-lookup"><span data-stu-id="20e9e-436">System.Object</span></span>

<span data-ttu-id="20e9e-437">웹 요청의 본문을로 파이프 할 수 있습니다 `Invoke-RestMethod` .</span><span class="sxs-lookup"><span data-stu-id="20e9e-437">You can pipe the body of a web request to `Invoke-RestMethod`.</span></span>

## <span data-ttu-id="20e9e-438">출력</span><span class="sxs-lookup"><span data-stu-id="20e9e-438">OUTPUTS</span></span>

### <span data-ttu-id="20e9e-439">System.object, System.string, System.Xml.Xml문서</span><span class="sxs-lookup"><span data-stu-id="20e9e-439">System.Int64, System.String, System.Xml.XmlDocument</span></span>

<span data-ttu-id="20e9e-440">cmdlet의 출력은 검색된 콘텐츠의 형식에 따라 달라집니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-440">The output of the cmdlet depends upon the format of the content that is retrieved.</span></span>

### <span data-ttu-id="20e9e-441">PSObject</span><span class="sxs-lookup"><span data-stu-id="20e9e-441">PSObject</span></span>

<span data-ttu-id="20e9e-442">요청에서 JSON 문자열을 반환 하는 경우는 `Invoke-RestMethod` 문자열을 나타내는 **PSObject** 를 반환 합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-442">If the request returns JSON strings, `Invoke-RestMethod` returns a **PSObject** that represents the strings.</span></span>

## <span data-ttu-id="20e9e-443">참고</span><span class="sxs-lookup"><span data-stu-id="20e9e-443">NOTES</span></span>

<span data-ttu-id="20e9e-444">일부 플랫폼에서는 일부 기능을 사용 하지 못할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-444">Some features may not be available on all platforms.</span></span>

<span data-ttu-id="20e9e-445">.NET Core 3.1의 변경 내용으로 인해 PowerShell 7.0 이상에서는 [Httpclient. defaultproxy](/dotnet/api/system.net.http.httpclient.defaultproxy?view=netcore-3.1) 속성을 사용 하 여 프록시 구성을 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-445">Because of changes in .NET Core 3.1, PowerShell 7.0 and higher use the [HttpClient.DefaultProxy](/dotnet/api/system.net.http.httpclient.defaultproxy?view=netcore-3.1) Property to determine the proxy configuration.</span></span>

<span data-ttu-id="20e9e-446">이 속성의 값은 플랫폼에 따라 달라 지는 규칙입니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-446">The value of this property is different rules depending on your platform:</span></span>

- <span data-ttu-id="20e9e-447">**Windows의** 경우: 환경 변수 또는 사용자의 프록시 설정에서 프록시 구성을 읽습니다 (정의 되지 않은 경우).</span><span class="sxs-lookup"><span data-stu-id="20e9e-447">**For Windows** : Reads proxy configuration from environment variables or, if those are not defined, from the user's proxy settings.</span></span>
- <span data-ttu-id="20e9e-448">**MacOS의 경우** : 환경 변수 또는 시스템의 프록시 설정에서 프록시 구성을 읽습니다 (정의 되지 않은 경우).</span><span class="sxs-lookup"><span data-stu-id="20e9e-448">**For macOS** : Reads proxy configuration from environment variables or, if those are not defined, from the system's proxy settings.</span></span>
- <span data-ttu-id="20e9e-449">**Linux의** 경우: 환경 변수 또는가 정의 되지 않은 경우이 속성은 모든 주소를 우회 하는 구성 되지 않은 인스턴스를 초기화 합니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-449">**For Linux** : Reads proxy configuration from environment variables or, in case those are not defined, this property initializes a non-configured instance that bypasses all addresses.</span></span>

<span data-ttu-id="20e9e-450">`DefaultProxy`Windows 및 Unix 기반 플랫폼에서 초기화 하는 데 사용 되는 환경 변수는 다음과 같습니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-450">The environment variables used for `DefaultProxy` initialization on Windows and Unix-based platforms are:</span></span>

- <span data-ttu-id="20e9e-451">` HTTP_PROXY`: HTTP 요청에 사용 되는 프록시 서버의 호스트 이름 또는 IP 주소입니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-451">` HTTP_PROXY`: the hostname or IP address of the proxy server used on HTTP requests.</span></span>
- <span data-ttu-id="20e9e-452">`HTTPS_PROXY`: HTTPS 요청에 사용 되는 프록시 서버의 호스트 이름 또는 IP 주소입니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-452">`HTTPS_PROXY`: the hostname or IP address of the proxy server used on HTTPS requests.</span></span>
- <span data-ttu-id="20e9e-453">`ALL_PROXY`: 또는의 경우 HTTP 및 HTTPS 요청에 사용 되는 프록시 서버의 호스트 이름 또는 IP 주소를 `HTTP_PROXY` `HTTPS_PROXY` 정의 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-453">`ALL_PROXY`: the hostname or IP address of the proxy server used on HTTP and HTTPS requests in case `HTTP_PROXY` or `HTTPS_PROXY` are not defined.</span></span>
- <span data-ttu-id="20e9e-454">`NO_PROXY`: 프록시에서 제외 되어야 하는 쉼표로 구분 된 호스트 이름 목록입니다.</span><span class="sxs-lookup"><span data-stu-id="20e9e-454">`NO_PROXY`: a comma-separated list of hostnames that should be excluded from proxying.</span></span>

## <span data-ttu-id="20e9e-455">관련 링크</span><span class="sxs-lookup"><span data-stu-id="20e9e-455">RELATED LINKS</span></span>

[<span data-ttu-id="20e9e-456">ConvertTo-Json</span><span class="sxs-lookup"><span data-stu-id="20e9e-456">ConvertTo-Json</span></span>](ConvertTo-Json.md)

[<span data-ttu-id="20e9e-457">ConvertFrom-Json</span><span class="sxs-lookup"><span data-stu-id="20e9e-457">ConvertFrom-Json</span></span>](ConvertFrom-Json.md)

[<span data-ttu-id="20e9e-458">Invoke-WebRequest</span><span class="sxs-lookup"><span data-stu-id="20e9e-458">Invoke-WebRequest</span></span>](Invoke-WebRequest.md)
