...
| **PSEdition**             | 이 속성의 값은  |
|                           | Powershell 4 이상 및 PowerShell  |
|                           | 모든 기능을 갖춘 Windows 버전에서 5.1.        |
|                           | 이 속성의 값은     |
|                           | Powershell 6 이상 (PowerShell)  |
|                           | 버전 축소 버전의 PowerShell 5.1  |
|                           | Windows Nano Server 또는 Windows IoT와 같습니다.      |
| **GitCommitId**           | GitHub에서 원본 파일의 커밋 Id |
| **OS**                    | 운영 체제에 대 한 설명입니다.      |
|                           | PowerShell이에서 실행 중입니다.                     |
| **플랫폼**              | 운영 체제에서 실행 중인 플랫폼 |
|                           | 켜짐: Linux 및 macOS의 값은 **Unix** 입니다. |
|                           | `$IsMacOs` 및 `$IsLinux`를 확인합니다.                |
