...
### <span data-ttu-id="d8157-109">세션</span><span class="sxs-lookup"><span data-stu-id="d8157-109">Session</span></span>
...
### <span data-ttu-id="d8157-119">ContainerId</span><span class="sxs-lookup"><span data-stu-id="d8157-119">ContainerId</span></span>
...
## <span data-ttu-id="d8157-141">예제</span><span class="sxs-lookup"><span data-stu-id="d8157-141">EXAMPLES</span></span>
...
<span data-ttu-id="d8157-154">이 예에서는 `Get-Culture` Server02 이라는 원격 컴퓨터에서 영구 연결을 사용 하 여 세션에서 동일한 명령을 실행 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-154">This example runs the same `Get-Culture` command in a session, using a persistent connection, on the remote computer named Server02.</span></span>
...
<span data-ttu-id="d8157-166">**ComputerName** 매개 변수를 사용 하는 경우 PowerShell은 명령을 실행할 새 세션을 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-166">When you use the **ComputerName** parameter, PowerShell creates a new session to run the command.</span></span>
...
<span data-ttu-id="d8157-179">이 예제에서는를 사용 하 여 `Invoke-Command` 여러 컴퓨터에서 단일 명령을 실행 하는 방법을 보여 줍니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-179">This example demonstrates how to use `Invoke-Command` to run a single command on multiple computers.</span></span>
...
### <span data-ttu-id="d8157-216">예 10: 컴퓨터 이름 숨기기</span><span class="sxs-lookup"><span data-stu-id="d8157-216">Example 10: Hide the computer name</span></span>
...
<span data-ttu-id="d8157-227">키워드에 대 한 자세한 내용은 `Param` [about_Language_Keywords](./about/about_language_keywords.md#param)를 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="d8157-227">For more information about the `Param` keyword, see [about_Language_Keywords](./about/about_language_keywords.md#param).</span></span>
...
### <span data-ttu-id="d8157-270">예제 17: 원격 세션에서 네트워크 공유 액세스</span><span class="sxs-lookup"><span data-stu-id="d8157-270">Example 17: Access a network share in a remote session</span></span>
...
<span data-ttu-id="d8157-281">`Invoke-Command`이 cmdlet은 변수를 사용 하 여 `$s` 원격 컴퓨터 Server02에 연결 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-281">The `Invoke-Command` cmdlet uses the `$s` variable to connect to the remote computer, Server02.</span></span> <span data-ttu-id="d8157-282">**ScriptBlock** 매개 변수는 `Enable-WSManCredSSP` 원격 컴퓨터에서 실행 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-282">The **ScriptBlock** parameter runs `Enable-WSManCredSSP` on the remote computer.</span></span> <span data-ttu-id="d8157-283">**Role** 매개 변수는 원격 컴퓨터에서 CredSSP 서버 설정을 구성 하는 **서버** 를 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-283">The **Role** parameter specifies **Server** to configure the CredSSP server setting on the remote computer.</span></span>
...
<span data-ttu-id="d8157-296">**Sessionoption** 값은 해시 테이블입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-296">The value of **SessionOption** is a hash table.</span></span> <span data-ttu-id="d8157-297">**OutputBufferingMode** 값은 **Drop** 으로 설정 되 고 **IdleTimeout** 값은 **4320만** 밀리초 (12 시간)로 설정 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-297">The **OutputBufferingMode** value is set to **Drop** and the **IdleTimeout** value is set to **43200000** milliseconds (12 hours).</span></span>
...
### <span data-ttu-id="d8157-318">-ApplicationName</span><span class="sxs-lookup"><span data-stu-id="d8157-318">-ApplicationName</span></span>
...
<span data-ttu-id="d8157-333">**Argumentlist** 매개 변수의 값은 1024 같은 실제 값 이거나와 같은 지역 변수에 대 한 참조일 수 있습니다 `$max` .</span><span class="sxs-lookup"><span data-stu-id="d8157-333">The values in the **ArgumentList** parameter can be actual values, such as 1024, or they can be references to local variables, such as `$max`.</span></span>
...
<span data-ttu-id="d8157-361">기본값은 Default입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-361">The default value is Default.</span></span>
...
<span data-ttu-id="d8157-368">연결이 끊긴 세션에 연결할 권한이 있는 사용자 계정의 디지털 공개 키 인증서(X509)를 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-368">Specifies the digital public key certificate (X509) of a user account that has permission to connect to the disconnected session.</span></span> <span data-ttu-id="d8157-369">인증서의 인증서 지문을 입력합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-369">Enter the certificate thumbprint of the certificate.</span></span>

<span data-ttu-id="d8157-370">인증서는 클라이언트 인증서 기반 인증에 사용됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-370">Certificates are used in client certificate-based authentication.</span></span> <span data-ttu-id="d8157-371">로컬 사용자 계정에만 매핑할 수 있으며 도메인 계정에는 사용할 수 없습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-371">They can be mapped only to local user accounts and they don't work with domain accounts.</span></span>

<span data-ttu-id="d8157-372">인증서 지문을 가져오려면 `Get-Item` `Get-ChildItem` PowerShell Cert: 드라이브에서 또는 명령을 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-372">To get a certificate thumbprint, use a `Get-Item` or `Get-ChildItem` command in the PowerShell Cert: drive.</span></span>
...
### <span data-ttu-id="d8157-373">-ComputerName</span><span class="sxs-lookup"><span data-stu-id="d8157-373">-ComputerName</span></span>

<span data-ttu-id="d8157-374">명령이 실행되는 컴퓨터를 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-374">Specifies the computers on which the command runs.</span></span> <span data-ttu-id="d8157-375">기본값은 로컬 컴퓨터입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-375">The default is the local computer.</span></span>

<span data-ttu-id="d8157-376">**ComputerName** 매개 변수를 사용 하는 경우 PowerShell은 지정 된 명령을 실행 하는 데만 사용 되 고 닫힌 임시 연결을 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-376">When you use the **ComputerName** parameter, PowerShell creates a temporary connection that's used only to run the specified command and is then closed.</span></span> <span data-ttu-id="d8157-377">영구 연결이 필요 하면 **Session** 매개 변수를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-377">If you need a persistent connection, use the **Session** parameter.</span></span>

<span data-ttu-id="d8157-378">하나 이상 컴퓨터의 NetBIOS 이름, IP 주소 또는 정규화된 도메인 이름을 쉼표로 구분된 목록으로 입력합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-378">Type the NETBIOS name, IP address, or fully qualified domain name of one or more computers in a comma-separated list.</span></span> <span data-ttu-id="d8157-379">로컬 컴퓨터를 지정 하려면 컴퓨터 이름, localhost 또는 점 ()을 입력 `.` 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-379">To specify the local computer, type the computer name, localhost, or a dot (`.`).</span></span>

<span data-ttu-id="d8157-380">**ComputerName** 값에 IP 주소를 사용 하려면 명령에 **Credential** 매개 변수를 포함 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-380">To use an IP address in the value of **ComputerName** , the command must include the **Credential** parameter.</span></span> <span data-ttu-id="d8157-381">컴퓨터를 HTTPS 전송에 대해 구성 하거나 원격 컴퓨터의 IP 주소를 로컬 컴퓨터의 WinRM **TrustedHosts** 목록에 포함 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-381">The computer must be configured for the HTTPS transport or the IP address of the remote computer must be included in the local computer's WinRM **TrustedHosts** list.</span></span> <span data-ttu-id="d8157-382">**TrustedHosts** 목록에 컴퓨터 이름을 추가 하는 방법에 대 한 지침은 [신뢰할 수 있는 호스트 목록에 컴퓨터를 추가 하는 방법](./about/about_remote_troubleshooting.md#how-to-add-a-computer-to-the-trusted-hosts-list)을 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="d8157-382">For instructions to add a computer name to the **TrustedHosts** list, see [How to Add a Computer to the Trusted Host List](./about/about_remote_troubleshooting.md#how-to-add-a-computer-to-the-trusted-hosts-list).</span></span>

<span data-ttu-id="d8157-383">Windows Vista 이상 버전의 Windows 운영 체제에서 로컬 컴퓨터를 **ComputerName** 값에 포함 하려면 **관리자 권한으로 실행** 옵션을 사용 하 여 PowerShell을 실행 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-383">On Windows Vista and later versions of the Windows operating system, to include the local computer in the value of **ComputerName** , you must run PowerShell using the **Run as administrator** option.</span></span>
...
### <span data-ttu-id="d8157-384">-ConfigurationName</span><span class="sxs-lookup"><span data-stu-id="d8157-384">-ConfigurationName</span></span>

<span data-ttu-id="d8157-385">새 **PSSession** 에 사용 되는 세션 구성을 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-385">Specifies the session configuration that is used for the new **PSSession**.</span></span>

<span data-ttu-id="d8157-386">세션 구성의 구성 이름 또는 정규화된 리소스 URI를 입력합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-386">Enter a configuration name or the fully qualified resource URI for a session configuration.</span></span> <span data-ttu-id="d8157-387">구성 이름만 지정 하는 경우에는 다음 스키마 URI가 앞에와 야 `http://schemas.microsoft.com/PowerShell` 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-387">If you specify only the configuration name, the following schema URI is prepended: `http://schemas.microsoft.com/PowerShell`.</span></span>

<span data-ttu-id="d8157-388">SSH와 함께 사용 하는 경우이 매개 변수는에 정의 된 대로 대상에서 사용할 하위 시스템을 지정 합니다 `sshd_config` .</span><span class="sxs-lookup"><span data-stu-id="d8157-388">When used with SSH, this parameter specifies the subsystem to use on the target as defined in `sshd_config`.</span></span> <span data-ttu-id="d8157-389">SSH의 기본값은 `powershell` 하위 시스템입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-389">The default value for SSH is the `powershell` subsystem.</span></span>

<span data-ttu-id="d8157-390">세션의 세션 구성은 원격 컴퓨터에 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-390">The session configuration for a session is located on the remote computer.</span></span> <span data-ttu-id="d8157-391">지정 된 세션 구성이 원격 컴퓨터에 없으면 명령이 실패 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-391">If the specified session configuration doesn't exist on the remote computer, the command fails.</span></span>

<span data-ttu-id="d8157-392">기본값은 `$PSSessionConfigurationName` 로컬 컴퓨터의 기본 설정 변수 값입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-392">The default value is the value of the `$PSSessionConfigurationName` preference variable on the local computer.</span></span> <span data-ttu-id="d8157-393">이 기본 설정 변수가 설정 되지 않은 경우 기본값은 **Microsoft. PowerShell** 입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-393">If this preference variable isn't set, the default is **Microsoft.PowerShell**.</span></span> <span data-ttu-id="d8157-394">자세한 내용은 [about_Preference_Variables](about/about_Preference_Variables.md)를 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="d8157-394">For more information, see [about_Preference_Variables](about/about_Preference_Variables.md).</span></span>
...
### <span data-ttu-id="d8157-395">-ConnectionUri</span><span class="sxs-lookup"><span data-stu-id="d8157-395">-ConnectionUri</span></span>

<span data-ttu-id="d8157-396">세션에 대한 연결 엔드포인트를 정의하는 URI(Uniform Resource Identifier)를 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-396">Specifies a Uniform Resource Identifier (URI) that defines the connection endpoint of the session.</span></span>
<span data-ttu-id="d8157-397">URI는 정규화된 URI여야 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-397">The URI must be fully qualified.</span></span>

<span data-ttu-id="d8157-398">이 문자열의 형식은 다음과 같습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-398">The format of this string is as follows:</span></span>

`<Transport>://<ComputerName>:<Port>/<ApplicationName>`

<span data-ttu-id="d8157-399">기본값은 다음과 같습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-399">The default value is as follows:</span></span>

`http://localhost:5985/WSMAN`

<span data-ttu-id="d8157-400">연결 URI를 지정 하지 않으면 **UseSSL** 및 **Port** 매개 변수를 사용 하 여 연결 uri 값을 지정할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-400">If you don't specify a connection URI, you can use the **UseSSL** and **Port** parameters to specify the connection URI values.</span></span>

<span data-ttu-id="d8157-401">URI의 **Transport** 세그먼트에 유효한 값은 HTTP 및 HTTPS입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-401">Valid values for the **Transport** segment of the URI are HTTP and HTTPS.</span></span> <span data-ttu-id="d8157-402">전송 세그먼트를 사용 하 여 연결 URI를 지정 하 고 포트를 지정 하지 않으면 세션은 표준 포트 80 (HTTP의 경우, HTTPS의 경우 443)를 사용 하 여 만들어집니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-402">If you specify a connection URI with a Transport segment, but don't specify a port, the session is created with the standards ports: 80 for HTTP and 443 for HTTPS.</span></span> <span data-ttu-id="d8157-403">PowerShell 원격을 위한 기본 포트를 사용 하려면 HTTP의 경우 포트 5985을, HTTPS의 경우 5986을 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-403">To use the default ports for PowerShell remoting, specify port 5985 for HTTP or 5986 for HTTPS.</span></span>

<span data-ttu-id="d8157-404">대상 컴퓨터에서 연결을 다른 URI로 리디렉션하는 경우 명령에서 **allowredirection** 매개 변수를 사용 하지 않으면 PowerShell에서 리디렉션을 방지 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-404">If the destination computer redirects the connection to a different URI, PowerShell prevents the redirection unless you use the **AllowRedirection** parameter in the command.</span></span>
...
### <span data-ttu-id="d8157-405">-ContainerId</span><span class="sxs-lookup"><span data-stu-id="d8157-405">-ContainerId</span></span>

<span data-ttu-id="d8157-406">컨테이너 Id의 배열을 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-406">Specifies an array of container IDs.</span></span>
...
### <span data-ttu-id="d8157-407">-Credential</span><span class="sxs-lookup"><span data-stu-id="d8157-407">-Credential</span></span>

<span data-ttu-id="d8157-408">이 작업을 수행할 권한이 있는 사용자 계정을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-408">Specifies a user account that has permission to perform this action.</span></span> <span data-ttu-id="d8157-409">기본값은 현재 사용자입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-409">The default is the current user.</span></span>

<span data-ttu-id="d8157-410">**User01** 또는 **Domain01\User01** 과 같은 사용자 이름을 입력 하거나 cmdlet에 의해 생성 된 **PSCredential** 개체를 입력 합니다 `Get-Credential` .</span><span class="sxs-lookup"><span data-stu-id="d8157-410">Type a user name, such as **User01** or **Domain01\User01** , or enter a **PSCredential** object generated by the `Get-Credential` cmdlet.</span></span> <span data-ttu-id="d8157-411">사용자 이름을 입력 하면 암호를 입력 하 라는 메시지가 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-411">If you type a user name, you're prompted to enter the password.</span></span>

<span data-ttu-id="d8157-412">자격 증명은 [PSCredential](/dotnet/api/system.management.automation.pscredential) 개체에 저장 되 고 암호는 [SecureString](/dotnet/api/system.security.securestring)으로 저장 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-412">Credentials are stored in a [PSCredential](/dotnet/api/system.management.automation.pscredential) object and the password is stored as a [SecureString](/dotnet/api/system.security.securestring).</span></span>

> [!NOTE]
> <span data-ttu-id="d8157-413">**Securestring** 데이터 보호에 대 한 자세한 [내용은 참조 하십시오](/dotnet/api/system.security.securestring#how-secure-is-securestring).</span><span class="sxs-lookup"><span data-stu-id="d8157-413">For more information about **SecureString** data protection, see [How secure is SecureString?](/dotnet/api/system.security.securestring#how-secure-is-securestring).</span></span>
...
### <span data-ttu-id="d8157-414">-EnableNetworkAccess</span><span class="sxs-lookup"><span data-stu-id="d8157-414">-EnableNetworkAccess</span></span>

<span data-ttu-id="d8157-415">이 cmdlet이 루프백 세션에 대화형 보안 토큰을 추가 함을 나타냅니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-415">Indicates that this cmdlet adds an interactive security token to loopback sessions.</span></span> <span data-ttu-id="d8157-416">대화형 토큰을 사용하면 다른 컴퓨터에서 데이터를 가져오는 명령을 루프백 세션에서 실행할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-416">The interactive token lets you run commands in the loopback session that get data from other computers.</span></span> <span data-ttu-id="d8157-417">예를 들어 원격 컴퓨터에서 로컬 컴퓨터로 XML 파일을 복사하는 세션에서 명령을 실행할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-417">For example, you can run a command in the session that copies XML files from a remote computer to the local computer.</span></span>

<span data-ttu-id="d8157-418">루프백 세션은 동일한 컴퓨터에서 시작 하 여 종료 되는 **PSSession** 입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-418">A loopback session is a **PSSession** that originates and ends on the same computer.</span></span> <span data-ttu-id="d8157-419">루프백 세션을 만들려면 **ComputerName** 매개 변수를 생략 하거나 해당 값을 점 ( `.` ), localhost 또는 로컬 컴퓨터 이름으로 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-419">To create a loopback session, omit the **ComputerName** parameter or set its value to dot (`.`), localhost, or the name of the local computer.</span></span>

<span data-ttu-id="d8157-420">기본적으로 루프백 세션은 원격 컴퓨터를 인증 하는 데 충분 한 권한을 제공 하지 않을 수 있는 네트워크 토큰을 사용 하 여 생성 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-420">By default, loopback sessions are created using a network token, which might not provide sufficient permission to authenticate to remote computers.</span></span>

<span data-ttu-id="d8157-421">**EnableNetworkAccess** 매개 변수는 루프백 세션에서만 유효합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-421">The **EnableNetworkAccess** parameter is effective only in loopback sessions.</span></span> <span data-ttu-id="d8157-422">원격 컴퓨터에서 세션을 만들 때 **EnableNetworkAccess** 를 사용 하는 경우 명령은 성공 하지만 매개 변수는 무시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-422">If you use **EnableNetworkAccess** when you create a session on a remote computer, the command succeeds, but the parameter is ignored.</span></span>

<span data-ttu-id="d8157-423">세션 자격 증명을 다른 컴퓨터에 위임 하는 **인증** 매개 변수의 **CredSSP** 값을 사용 하 여 루프백 세션에서 원격 액세스를 허용할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-423">You can allow remote access in a loopback session using the **CredSSP** value of the **Authentication** parameter, which delegates the session credentials to other computers.</span></span>

<span data-ttu-id="d8157-424">악의적인 액세스 로부터 컴퓨터를 보호 하기 위해 **EnableNetworkAccess** 를 사용 하 여 만든 대화형 토큰을 포함 하는 분리 된 루프백 세션은 세션을 만든 컴퓨터 에서만 다시 연결할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-424">To protect the computer from malicious access, disconnected loopback sessions that have interactive tokens, which are those created using **EnableNetworkAccess** , can be reconnected only from the computer on which the session was created.</span></span> <span data-ttu-id="d8157-425">CredSSP 인증을 사용하는 연결이 끊어진 세션은 다른 컴퓨터에서 다시 연결할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-425">Disconnected sessions that use CredSSP authentication can be reconnected from other computers.</span></span> <span data-ttu-id="d8157-426">자세한 내용은 `Disconnect-PSSession`를 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="d8157-426">For more information, see `Disconnect-PSSession`.</span></span>

<span data-ttu-id="d8157-427">이 매개 변수는 PowerShell 3.0에서 도입 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-427">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="d8157-428">-FilePath</span><span class="sxs-lookup"><span data-stu-id="d8157-428">-FilePath</span></span>

<span data-ttu-id="d8157-429">하나 이상의 원격 컴퓨터에서이 cmdlet이 실행 되는 로컬 스크립트를 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-429">Specifies a local script that this cmdlet runs on one or more remote computers.</span></span> <span data-ttu-id="d8157-430">스크립트의 경로와 파일 이름을 입력 하거나에 대 한 스크립트 경로를 파이프 `Invoke-Command` 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-430">Enter the path and filename of the script, or pipe a script path to `Invoke-Command`.</span></span> <span data-ttu-id="d8157-431">스크립트는 로컬 컴퓨터 또는 로컬 컴퓨터에서 액세스할 수 있는 디렉터리에 있어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-431">The script must exist on the local computer or in a directory that the local computer can access.</span></span> <span data-ttu-id="d8157-432">**Argumentlist** 를 사용 하 여 스크립트의 매개 변수 값을 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-432">Use **ArgumentList** to specify the values of parameters in the script.</span></span>

<span data-ttu-id="d8157-433">이 매개 변수를 사용 하는 경우 PowerShell은 지정 된 스크립트 파일의 내용을 스크립트 블록으로 변환 하 고 스크립트 블록을 원격 컴퓨터로 전송한 다음 원격 컴퓨터에서 실행 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-433">When you use this parameter, PowerShell converts the contents of the specified script file to a script block, transmits the script block to the remote computer, and runs it on the remote computer.</span></span>
...
### <span data-ttu-id="d8157-434">-HideComputerName</span><span class="sxs-lookup"><span data-stu-id="d8157-434">-HideComputerName</span></span>

<span data-ttu-id="d8157-435">이 cmdlet이 출력 표시에서 각 개체의 컴퓨터 이름을 생략 함을 나타냅니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-435">Indicates that this cmdlet omits the computer name of each object from the output display.</span></span> <span data-ttu-id="d8157-436">기본적으로 개체를 생성한 컴퓨터의 이름은 화면에 표시됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-436">By default, the name of the computer that generated the object appears in the display.</span></span>

<span data-ttu-id="d8157-437">이 매개 변수는 출력 표시에만 영향을 줍니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-437">This parameter affects only the output display.</span></span> <span data-ttu-id="d8157-438">개체를 변경 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-438">It doesn't change the object.</span></span>
...
### <span data-ttu-id="d8157-439">-HostName</span><span class="sxs-lookup"><span data-stu-id="d8157-439">-HostName</span></span>

<span data-ttu-id="d8157-440">Secure Shell (SSH) 기반 연결에 대 한 컴퓨터 이름 배열을 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-440">Specifies an array of computer names for a Secure Shell (SSH) based connection.</span></span> <span data-ttu-id="d8157-441">이는 원격 컴퓨터에 대 한 연결이 Windows WinRM이 아닌 SSH를 사용 하는 경우를 제외 하 고 **ComputerName** 매개 변수와 비슷합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-441">This is similar to the **ComputerName** parameter except that the connection to the remote computer is made using SSH rather than Windows WinRM.</span></span>

<span data-ttu-id="d8157-442">이 매개 변수는 PowerShell 6.0에서 도입 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-442">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="d8157-443">-InDisconnectedSession</span><span class="sxs-lookup"><span data-stu-id="d8157-443">-InDisconnectedSession</span></span>

<span data-ttu-id="d8157-444">이 cmdlet이 연결 되지 않은 세션에서 명령 또는 스크립트를 실행 함을 나타냅니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-444">Indicates that this cmdlet runs a command or script in a disconnected session.</span></span>

<span data-ttu-id="d8157-445">**InDisconnectedSession** 매개 변수를 사용 하는 경우는 `Invoke-Command` 각 원격 컴퓨터에 영구 세션을 만들고, **ScriptBlock** 또는 **FilePath** 매개 변수로 지정 된 명령을 시작한 다음 세션에서 연결을 끊습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-445">When you use the **InDisconnectedSession** parameter, `Invoke-Command` creates a persistent session on each remote computer, starts the command specified by the **ScriptBlock** or **FilePath** parameter, and then disconnects from the session.</span></span> <span data-ttu-id="d8157-446">명령은 연결 되지 않은 세션에서 계속 실행 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-446">The commands continue to run in the disconnected sessions.</span></span> <span data-ttu-id="d8157-447">**InDisconnectedSession** 를 사용 하면 원격 세션에 대 한 연결을 유지 하지 않고 명령을 실행할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-447">**InDisconnectedSession** enables you to run commands without maintaining a connection to the remote sessions.</span></span> <span data-ttu-id="d8157-448">결과를 반환 하기 전에 세션의 연결이 끊어져서 **InDisconnectedSession** 는 세션 간에 분할 되지 않고 다시 연결 된 세션에 모든 명령 결과가 반환 되도록 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-448">And, because the session is disconnected before any results are returned, **InDisconnectedSession** makes sure that all command results are returned to the reconnected session, instead of being split between sessions.</span></span>

<span data-ttu-id="d8157-449">**InDisconnectedSession** 은 **Session** 매개 변수 또는 **AsJob** 매개 변수와 함께 사용할 수 없습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-449">You can't use **InDisconnectedSession** with the **Session** parameter or the **AsJob** parameter.</span></span>

<span data-ttu-id="d8157-450">**InDisconnectedSession** 를 사용 하는 명령은 연결 되지 않은 세션을 나타내는 **PSSession** 개체를 반환 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-450">Commands that use **InDisconnectedSession** return a **PSSession** object that represents the disconnected session.</span></span> <span data-ttu-id="d8157-451">명령 출력을 반환 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-451">They don't return the command output.</span></span> <span data-ttu-id="d8157-452">연결이 끊어진 세션에 연결 하려면 `Connect-PSSession` 또는 cmdlet을 사용 `Receive-PSSession` 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-452">To connect to the disconnected session, use the `Connect-PSSession` or `Receive-PSSession` cmdlets.</span></span> <span data-ttu-id="d8157-453">세션에서 실행 된 명령의 결과를 가져오려면 cmdlet을 사용 합니다 `Receive-PSSession` .</span><span class="sxs-lookup"><span data-stu-id="d8157-453">To get the results of commands that ran in the session, use the `Receive-PSSession` cmdlet.</span></span> <span data-ttu-id="d8157-454">연결 되지 않은 세션에서 출력을 생성 하는 명령을 실행 하려면 **OutputBufferingMode** 세션 옵션의 값을 **Drop** 으로 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-454">To run commands that generate output in a disconnected session, set the value of the **OutputBufferingMode** session option to **Drop**.</span></span> <span data-ttu-id="d8157-455">연결이 끊어진 세션에 연결 하려면 세션을 삭제 하기 전에 연결 하는 데 충분 한 시간을 제공 하도록 세션의 유휴 제한 시간을 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-455">If you intend to connect to the disconnected session, set the idle time-out in the session so that it provides sufficient time for you to connect before deleting the session.</span></span>

<span data-ttu-id="d8157-456">**Sessionoption** 매개 변수 또는 기본 설정 변수에서 출력 버퍼링 모드 및 유휴 제한 시간을 설정할 수 있습니다 `$PSSessionOption` .</span><span class="sxs-lookup"><span data-stu-id="d8157-456">You can set the output buffering mode and idle time-out in the **SessionOption** parameter or in the `$PSSessionOption` preference variable.</span></span> <span data-ttu-id="d8157-457">세션 옵션에 대 한 자세한 내용은 `New-PSSessionOption` 및 [about_Preference_Variables](./about/about_preference_variables.md)을 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="d8157-457">For more information about session options, see `New-PSSessionOption` and [about_Preference_Variables](./about/about_preference_variables.md).</span></span>

<span data-ttu-id="d8157-458">연결이 끊긴 세션 기능에 대한 자세한 내용은 [about_Remote_Disconnected_Sessions](about/about_Remote_Disconnected_Sessions.md)를 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="d8157-458">For more information about the Disconnected Sessions feature, see [about_Remote_Disconnected_Sessions](about/about_Remote_Disconnected_Sessions.md).</span></span>

<span data-ttu-id="d8157-459">이 매개 변수는 PowerShell 3.0에서 도입 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-459">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="d8157-460">-InputObject</span><span class="sxs-lookup"><span data-stu-id="d8157-460">-InputObject</span></span>

<span data-ttu-id="d8157-461">명령에 대한 입력을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-461">Specifies input to the command.</span></span> <span data-ttu-id="d8157-462">개체를 포함하는 변수를 입력하거나 개체를 가져오는 명령 또는 식을 입력하세요.</span><span class="sxs-lookup"><span data-stu-id="d8157-462">Enter a variable that contains the objects or type a command or expression that gets the objects.</span></span>

<span data-ttu-id="d8157-463">**InputObject** 매개 변수를 사용 하는 경우 `$Input` **ScriptBlock** 매개 변수 값의 자동 변수를 사용 하 여 입력 개체를 나타냅니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-463">When using the **InputObject** parameter, use the `$Input` automatic variable in the value of the **ScriptBlock** parameter to represent the input objects.</span></span>
...
### <span data-ttu-id="d8157-464">-JobName</span><span class="sxs-lookup"><span data-stu-id="d8157-464">-JobName</span></span>

<span data-ttu-id="d8157-465">백그라운드 작업의 이름을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-465">Specifies a friendly name for the background job.</span></span> <span data-ttu-id="d8157-466">기본적으로 작업의 이름은 이며 `Job<n>` , 여기서 `<n>` 는 서 수입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-466">By default, jobs are named `Job<n>`, where `<n>` is an ordinal number.</span></span>

<span data-ttu-id="d8157-467">명령에서 **JobName** 매개 변수를 사용 하는 경우 명령 `Invoke-Command` 에 **AsJob** 를 포함 하지 않는 경우에도 명령이 작업으로 실행 되 고 작업 개체를 반환 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-467">If you use the **JobName** parameter in a command, the command is run as a job, and `Invoke-Command` returns a job object, even if you don't include **AsJob** in the command.</span></span>

<span data-ttu-id="d8157-468">PowerShell 백그라운드 작업에 대 한 자세한 내용은 [about_Jobs](./About/about_Jobs.md)를 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="d8157-468">For more information about PowerShell background jobs, see [about_Jobs](./About/about_Jobs.md).</span></span>
...
### <span data-ttu-id="d8157-469">-KeyFilePath</span><span class="sxs-lookup"><span data-stu-id="d8157-469">-KeyFilePath</span></span>

<span data-ttu-id="d8157-470">SSH (Secure Shell)에서 원격 컴퓨터의 사용자를 인증 하는 데 사용 하는 키 파일 경로를 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-470">Specifies a key file path used by Secure Shell (SSH) to authenticate a user on a remote computer.</span></span>

<span data-ttu-id="d8157-471">SSH를 사용 하면 기본 암호 인증 대신 개인 및 공개 키를 통해 사용자 인증을 수행할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-471">SSH allows user authentication to be performed via private and public keys as an alternative to basic password authentication.</span></span> <span data-ttu-id="d8157-472">키 인증을 사용 하도록 원격 컴퓨터를 구성 하는 경우이 매개 변수를 사용 하 여 사용자를 식별 하는 키를 제공할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-472">If the remote computer is configured for key authentication, then this parameter can be used to provide the key that identifies the user.</span></span>

<span data-ttu-id="d8157-473">이 매개 변수는 PowerShell 6.0에서 도입 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-473">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="d8157-474">-NoNewScope</span><span class="sxs-lookup"><span data-stu-id="d8157-474">-NoNewScope</span></span>

<span data-ttu-id="d8157-475">이 cmdlet이 현재 범위의 지정 된 명령을 실행 함을 나타냅니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-475">Indicates that this cmdlet runs the specified command in the current scope.</span></span> <span data-ttu-id="d8157-476">기본적으로는 `Invoke-Command` 자체 범위에서 명령을 실행 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-476">By default, `Invoke-Command` runs commands in their own scope.</span></span>

<span data-ttu-id="d8157-477">이 매개 변수는 현재 세션에서 실행하는 명령 즉, **ComputerName** 및 **Session** 매개 변수를 모두 생략하는 명령에서만 유효합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-477">This parameter is valid only in commands that are run in the current session, that is, commands that omit both the **ComputerName** and **Session** parameters.</span></span>

<span data-ttu-id="d8157-478">이 매개 변수는 PowerShell 3.0에서 도입 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-478">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="d8157-479">-Port</span><span class="sxs-lookup"><span data-stu-id="d8157-479">-Port</span></span>

<span data-ttu-id="d8157-480">이 명령에 사용 되는 원격 컴퓨터의 네트워크 포트를 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-480">Specifies the network port on the remote computer that is used for this command.</span></span> <span data-ttu-id="d8157-481">원격 컴퓨터에 연결하려면 원격 컴퓨터가 연결에서 사용하는 포트에서 수신 대기하고 있어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-481">To connect to a remote computer, the remote computer must be listening on the port that the connection uses.</span></span> <span data-ttu-id="d8157-482">기본 포트는 5985 (HTTP의 경우 WinRM 포트) 및 5986 (HTTPS의 경우 WinRM 포트)입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-482">The default ports are 5985 (WinRM port for HTTP) and 5986 (WinRM port for HTTPS).</span></span>

<span data-ttu-id="d8157-483">대체 포트를 사용하기 전에 원격 컴퓨터가 해당 포트에서 수신하도록 원격 컴퓨터의 WinRM 수신기를 구성해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-483">Before using an alternate port, configure the WinRM listener on the remote computer to listen at that port.</span></span> <span data-ttu-id="d8157-484">수신기를 구성 하려면 PowerShell 프롬프트에 다음 두 명령을 입력 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-484">To configure the listener, type the following two commands at the PowerShell prompt:</span></span>

`Remove-Item -Path WSMan:\Localhost\listener\listener* -Recurse`

`New-Item -Path WSMan:\Localhost\listener -Transport http -Address * -Port \<port-number\>`

<span data-ttu-id="d8157-485">반드시 필요한 경우가 아니면 **Port** 매개 변수를 사용 하지 마세요.</span><span class="sxs-lookup"><span data-stu-id="d8157-485">Don't use the **Port** parameter unless you must.</span></span> <span data-ttu-id="d8157-486">이 명령에 설정된 포트는 명령이 실행되는 모든 컴퓨터나 세션에 적용됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-486">The port that is set in the command applies to all computers or sessions on which the command runs.</span></span> <span data-ttu-id="d8157-487">대체 포트 설정을 사용하면 일부 컴퓨터에서 명령이 실행되지 않을 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-487">An alternate port setting might prevent the command from running on all computers.</span></span>
...
### <span data-ttu-id="d8157-488">-RemoteDebug</span><span class="sxs-lookup"><span data-stu-id="d8157-488">-RemoteDebug</span></span>

<span data-ttu-id="d8157-489">호출 된 명령을 원격 PowerShell 세션의 디버그 모드에서 실행 하는 데 사용 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-489">Used to run the invoked command in debug mode in the remote PowerShell session.</span></span>
...
### <span data-ttu-id="d8157-490">-RunAsAdministrator</span><span class="sxs-lookup"><span data-stu-id="d8157-490">-RunAsAdministrator</span></span>

<span data-ttu-id="d8157-491">이 cmdlet이 명령을 관리자 권한으로 호출 함을 나타냅니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-491">Indicates that this cmdlet invokes a command as an Administrator.</span></span>
...
### <span data-ttu-id="d8157-492">-ScriptBlock</span><span class="sxs-lookup"><span data-stu-id="d8157-492">-ScriptBlock</span></span>

<span data-ttu-id="d8157-493">실행할 명령을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-493">Specifies the commands to run.</span></span> <span data-ttu-id="d8157-494">명령을 중괄호로 묶어 `{ }` 스크립트 블록을 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-494">Enclose the commands in curly braces `{ }` to create a script block.</span></span>
<span data-ttu-id="d8157-495">이 매개 변수는 필수적 요소입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-495">This parameter is required.</span></span>

<span data-ttu-id="d8157-496">기본적으로 명령에 있는 모든 변수는 원격 컴퓨터에서 평가됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-496">By default, any variables in the command are evaluated on the remote computer.</span></span> <span data-ttu-id="d8157-497">명령에 로컬 변수를 포함 하려면 **Argumentlist** 를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-497">To include local variables in the command, use **ArgumentList**.</span></span>
...
### <span data-ttu-id="d8157-498">-Session</span><span class="sxs-lookup"><span data-stu-id="d8157-498">-Session</span></span>

<span data-ttu-id="d8157-499">이 cmdlet이 명령을 실행 하는 세션의 배열을 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-499">Specifies an array of sessions in which this cmdlet runs the command.</span></span> <span data-ttu-id="d8157-500">**Pssession** 개체를 포함 하는 변수를 입력 하거나 **pssession** 개체를 만들거나 가져오는 명령 (예: 또는 명령)을 입력 합니다 `New-PSSession` `Get-PSSession` .</span><span class="sxs-lookup"><span data-stu-id="d8157-500">Enter a variable that contains **PSSession** objects or a command that creates or gets the **PSSession** objects, such as a `New-PSSession` or `Get-PSSession` command.</span></span>

<span data-ttu-id="d8157-501">**PSSession** 을 만들 때 PowerShell은 원격 컴퓨터에 대 한 영구 연결을 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-501">When you create a **PSSession** , PowerShell establishes a persistent connection to the remote computer.</span></span> <span data-ttu-id="d8157-502">**PSSession** 을 사용 하 여 데이터를 공유 하는 일련의 관련 명령을 실행 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-502">Use a **PSSession** to run a series of related commands that share data.</span></span> <span data-ttu-id="d8157-503">단일 명령이 나 일련의 관련 되지 않은 명령을 실행 하려면 **ComputerName** 매개 변수를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-503">To run a single command or a series of unrelated commands, use the **ComputerName** parameter.</span></span> <span data-ttu-id="d8157-504">자세한 내용은 [about_PSSessions](./About/about_PSSessions.md)를 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="d8157-504">For more information, see [about_PSSessions](./About/about_PSSessions.md).</span></span>
...
### <span data-ttu-id="d8157-505">-세션 이름</span><span class="sxs-lookup"><span data-stu-id="d8157-505">-SessionName</span></span>

<span data-ttu-id="d8157-506">연결이 끊긴 세션의 표시 이름을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-506">Specifies a friendly name for a disconnected session.</span></span> <span data-ttu-id="d8157-507">이 이름을 사용 하 여 명령 등의 후속 명령에서 세션을 참조할 수 있습니다 `Get-PSSession` .</span><span class="sxs-lookup"><span data-stu-id="d8157-507">You can use the name to refer to the session in subsequent commands, such as a `Get-PSSession` command.</span></span> <span data-ttu-id="d8157-508">이 매개 변수는 **InDisconnectedSession** 매개 변수와 함께 사용할 경우에만 유효합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-508">This parameter is valid only with the **InDisconnectedSession** parameter.</span></span>

<span data-ttu-id="d8157-509">이 매개 변수는 PowerShell 3.0에서 도입 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-509">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="d8157-510">-SessionOption</span><span class="sxs-lookup"><span data-stu-id="d8157-510">-SessionOption</span></span>

<span data-ttu-id="d8157-511">세션에 대 한 고급 옵션을 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-511">Specifies advanced options for the session.</span></span> <span data-ttu-id="d8157-512">Cmdlet을 사용 하 여 만든 것과 같은 **sessionoption** 개체를 입력 `New-PSSessionOption` 하거나 키가 세션 옵션 이름이 고 값이 session 옵션 값인 해시 테이블을 입력 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-512">Enter a **SessionOption** object, such as one that you create using the `New-PSSessionOption` cmdlet, or a hash table in which the keys are session option names and the values are session option values.</span></span>

<span data-ttu-id="d8157-513">옵션의 기본값은 설정 된 경우 기본 설정 변수의 값에 따라 결정 됩니다 `$PSSessionOption` .</span><span class="sxs-lookup"><span data-stu-id="d8157-513">The default values for the options are determined by the value of the `$PSSessionOption` preference variable, if it's set.</span></span> <span data-ttu-id="d8157-514">그러지 않으면 기본값은 세션 구성에 설정된 옵션에 따라 설정됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-514">Otherwise, the default values are established by options set in the session configuration.</span></span>

<span data-ttu-id="d8157-515">세션 옵션 값은 기본 설정 변수 및 세션 구성에 설정 된 세션의 기본값 보다 우선 적용 `$PSSessionOption` 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-515">The session option values take precedence over default values for sessions set in the `$PSSessionOption` preference variable and in the session configuration.</span></span> <span data-ttu-id="d8157-516">그러나 세션 구성에 설정 된 최대값, 할당량 또는 제한 보다 우선 적용 되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-516">However, they don't take precedence over maximum values, quotas, or limits set in the session configuration.</span></span>

<span data-ttu-id="d8157-517">기본값을 포함 하는 세션 옵션에 대 한 설명은를 참조 하십시오 `New-PSSessionOption` .</span><span class="sxs-lookup"><span data-stu-id="d8157-517">For a description of the session options that includes the default values, see `New-PSSessionOption`.</span></span> <span data-ttu-id="d8157-518">기본 설정 변수에 대 한 자세한 내용은 `$PSSessionOption` [about_Preference_Variables](About/about_Preference_Variables.md)를 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="d8157-518">For information about the `$PSSessionOption` preference variable, see [about_Preference_Variables](About/about_Preference_Variables.md).</span></span>
<span data-ttu-id="d8157-519">세션 구성에 대 한 자세한 내용은 [about_Session_Configurations](About/about_Session_Configurations.md)를 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="d8157-519">For more information about session configurations, see [about_Session_Configurations](About/about_Session_Configurations.md).</span></span>
...
### <span data-ttu-id="d8157-520">-SSHConnection</span><span class="sxs-lookup"><span data-stu-id="d8157-520">-SSHConnection</span></span>

<span data-ttu-id="d8157-521">이 매개 변수는 해시 테이블의 배열을 사용 합니다. 여기서 각 해시 테이블에는 SSH (Secure Shell) 연결을 설정 하는 데 필요한 하나 이상의 연결 매개 변수가 포함 되어 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-521">This parameter takes an array of hash tables where each hash table contains one or more connection parameters needed to establish a Secure Shell (SSH) connection.</span></span> <span data-ttu-id="d8157-522">**SSHConnection** 매개 변수는 각 세션에 다른 연결 정보가 필요한 여러 세션을 만드는 데 유용 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-522">The **SSHConnection** parameter is useful for creating multiple sessions where each session requires different connection information.</span></span>

<span data-ttu-id="d8157-523">해시 테이블에는 다음과 같은 멤버가 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-523">The hashtable has the following members:</span></span>

- <span data-ttu-id="d8157-524">**ComputerName** (또는 **HostName** )</span><span class="sxs-lookup"><span data-stu-id="d8157-524">**ComputerName** (or **HostName** )</span></span>
- <span data-ttu-id="d8157-525">**포트**</span><span class="sxs-lookup"><span data-stu-id="d8157-525">**Port**</span></span>
- <span data-ttu-id="d8157-526">**UserName**</span><span class="sxs-lookup"><span data-stu-id="d8157-526">**UserName**</span></span>
- <span data-ttu-id="d8157-527">**Keyfilepath** (또는 **IdentityFilePath** )</span><span class="sxs-lookup"><span data-stu-id="d8157-527">**KeyFilePath** (or **IdentityFilePath** )</span></span>

<span data-ttu-id="d8157-528">**ComputerName** (또는 **HostName** )은 필요한 유일한 키-값 쌍입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-528">**ComputerName** (or **HostName** ) is the only key-value pair that is required.</span></span>

<span data-ttu-id="d8157-529">이 매개 변수는 PowerShell 6.0에서 도입 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-529">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="d8157-530">-SSHTransport</span><span class="sxs-lookup"><span data-stu-id="d8157-530">-SSHTransport</span></span>

<span data-ttu-id="d8157-531">SSH (Secure Shell)를 사용 하 여 원격 연결을 설정 했음을 나타냅니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-531">Indicates that the remote connection is established using Secure Shell (SSH).</span></span>

<span data-ttu-id="d8157-532">기본적으로 PowerShell은 Windows WinRM을 사용 하 여 원격 컴퓨터에 연결 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-532">By default PowerShell uses Windows WinRM to connect to a remote computer.</span></span> <span data-ttu-id="d8157-533">이 스위치를 사용 하면 PowerShell에서 **호스트 이름** 매개 변수를 사용 하 여 SSH 기반 원격 연결을 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-533">This switch forces PowerShell to use the **HostName** parameter for establishing an SSH based remote connection.</span></span>

<span data-ttu-id="d8157-534">이 매개 변수는 PowerShell 6.0에서 도입 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-534">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="d8157-535">-ThrottleLimit</span><span class="sxs-lookup"><span data-stu-id="d8157-535">-ThrottleLimit</span></span>

<span data-ttu-id="d8157-536">이 명령을 실행하도록 설정할 수 있는 최대 동시 연결 수를 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-536">Specifies the maximum number of concurrent connections that can be established to run this command.</span></span>
<span data-ttu-id="d8157-537">이 매개 변수를 생략하거나 값 0을 입력하면 기본값 32가 사용됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-537">If you omit this parameter or enter a value of 0, the default value, 32, is used.</span></span>

<span data-ttu-id="d8157-538">제한 한도는 현재 명령에만 적용되며 세션이나 컴퓨터에는 적용되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-538">The throttle limit applies only to the current command, not to the session or to the computer.</span></span>
...
### <span data-ttu-id="d8157-539">-UserName</span><span class="sxs-lookup"><span data-stu-id="d8157-539">-UserName</span></span>

<span data-ttu-id="d8157-540">원격 컴퓨터에서 명령을 실행 하는 데 사용 되는 계정의 사용자 이름을 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-540">Specifies the username for the account used to run a command on the remote computer.</span></span> <span data-ttu-id="d8157-541">사용자 인증 방법은 원격 컴퓨터에서 SSH (Secure Shell)가 구성 된 방식에 따라 달라 집니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-541">The user authentication method depends on how Secure Shell (SSH) is configured on the remote computer.</span></span>

<span data-ttu-id="d8157-542">SSH가 기본 암호 인증으로 구성 된 경우 사용자 암호를 입력 하 라는 메시지가 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-542">If SSH is configured for basic password authentication, then you'll be prompted for the user password.</span></span>

<span data-ttu-id="d8157-543">키 기반 사용자 인증에 대해 SSH를 구성 하는 경우 키 파일 경로는 **Keyfilepath** 매개 변수를 통해 제공할 수 있으며 암호 프롬프트는 발생 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-543">If SSH is configured for key-based user authentication then a key file path can be provided via the **KeyFilePath** parameter and no password prompt occurs.</span></span> <span data-ttu-id="d8157-544">클라이언트 사용자 키 파일이 SSH의 알려진 위치에 있는 경우 키 기반 인증에는 **Keyfilepath** 매개 변수가 필요 하지 않으며 사용자 인증은 사용자 이름에 따라 자동으로 수행 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-544">If the client user key file is located in an SSH known location, then the **KeyFilePath** parameter isn't needed for key-based authentication, and user authentication occurs automatically based on the username.</span></span> <span data-ttu-id="d8157-545">자세한 내용은 키 기반 사용자 인증에 대 한 플랫폼의 SSH 설명서를 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="d8157-545">For more information, see your platform's SSH documentation about key-based user authentication.</span></span>

<span data-ttu-id="d8157-546">이 매개 변수는 필수 매개 변수가 아닙니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-546">This isn't a required parameter.</span></span> <span data-ttu-id="d8157-547">**UserName** 매개 변수를 지정 하지 않으면 현재 로그온 한 사용자 이름이 연결에 사용 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-547">If the **UserName** parameter isn't specified, then the current logged on username is used for the connection.</span></span>

<span data-ttu-id="d8157-548">이 매개 변수는 PowerShell 6.0에서 도입 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-548">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="d8157-549">-UseSSL</span><span class="sxs-lookup"><span data-stu-id="d8157-549">-UseSSL</span></span>

<span data-ttu-id="d8157-550">이 cmdlet이 SSL(Secure Sockets Layer) (SSL) 프로토콜을 사용 하 여 원격 컴퓨터에 대 한 연결을 설정 함을 나타냅니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-550">Indicates that this cmdlet uses the Secure Sockets Layer (SSL) protocol to establish a connection to the remote computer.</span></span> <span data-ttu-id="d8157-551">기본적으로 SSL은 사용 되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-551">By default, SSL isn't used.</span></span>

<span data-ttu-id="d8157-552">WS-Management는 네트워크를 통해 전송 되는 모든 PowerShell 콘텐츠를 암호화 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-552">WS-Management encrypts all PowerShell content transmitted over the network.</span></span> <span data-ttu-id="d8157-553">**UseSSL** 매개 변수는 HTTP 대신 HTTPS를 통해 데이터를 전송 하는 추가 보호 기능입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-553">The **UseSSL** parameter is an additional protection that sends the data across an HTTPS, instead of HTTP.</span></span>

<span data-ttu-id="d8157-554">이 매개 변수를 사용 하지만 명령에 사용 되는 포트에서 SSL을 사용할 수 없는 경우 명령이 실패 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-554">If you use this parameter, but SSL isn't available on the port that's used for the command, the command fails.</span></span>
...
### <span data-ttu-id="d8157-555">-VMId</span><span class="sxs-lookup"><span data-stu-id="d8157-555">-VMId</span></span>

<span data-ttu-id="d8157-556">가상 컴퓨터의 Id 배열을 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-556">Specifies an array of IDs of virtual machines.</span></span>
...
### <span data-ttu-id="d8157-557">-VMName</span><span class="sxs-lookup"><span data-stu-id="d8157-557">-VMName</span></span>

<span data-ttu-id="d8157-558">가상 컴퓨터의 이름 배열을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-558">Specifies an array of names of virtual machines.</span></span>
...
### <span data-ttu-id="d8157-559">-하위 시스템</span><span class="sxs-lookup"><span data-stu-id="d8157-559">-Subsystem</span></span>

<span data-ttu-id="d8157-560">새 **PSSession** 에 사용 되는 SSH 하위 시스템을 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-560">Specifies the SSH subsystem used for the new **PSSession**.</span></span>

<span data-ttu-id="d8157-561">이는 sshd_config에 정의 된 대로 대상에서 사용할 하위 시스템을 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-561">This specifies the subsystem to use on the target as defined in sshd_config.</span></span>
<span data-ttu-id="d8157-562">하위 시스템은 미리 정의 된 매개 변수를 사용 하 여 특정 버전의 PowerShell을 시작 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-562">The subsystem starts a specific version of PowerShell with predefined parameters.</span></span>
<span data-ttu-id="d8157-563">지정 된 하위 시스템이 원격 컴퓨터에 없는 경우 명령이 실패 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-563">If the specified subsystem does not exist on the remote computer, the command fails.</span></span>

<span data-ttu-id="d8157-564">이 매개 변수를 사용 하지 않는 경우 기본값은 ' powershell ' 하위 시스템입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-564">If this parameter is not used, the default is the 'powershell' subsystem.</span></span>
...
### <span data-ttu-id="d8157-565">CommonParameters</span><span class="sxs-lookup"><span data-stu-id="d8157-565">CommonParameters</span></span>

<span data-ttu-id="d8157-566">이 cmdlet 일반 매개 변수를 지원합니다. -Debug, -ErrorAction, -ErrorVariable, -InformationAction, -InformationVariable, -OutVariable, -OutBuffer, -PipelineVariable, -Verbose, -WarningAction 및 -WarningVariable.</span><span class="sxs-lookup"><span data-stu-id="d8157-566">This cmdlet supports the common parameters: -Debug, -ErrorAction, -ErrorVariable, -InformationAction, -InformationVariable, -OutVariable, -OutBuffer, -PipelineVariable, -Verbose, -WarningAction, and -WarningVariable.</span></span> <span data-ttu-id="d8157-567">자세한 내용은 [about_CommonParameters](https://go.microsoft.com/fwlink/?LinkID=113216)를 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="d8157-567">For more information, see [about_CommonParameters](https://go.microsoft.com/fwlink/?LinkID=113216).</span></span>

## <span data-ttu-id="d8157-568">입력</span><span class="sxs-lookup"><span data-stu-id="d8157-568">INPUTS</span></span>

### <span data-ttu-id="d8157-569">System.object.</span><span class="sxs-lookup"><span data-stu-id="d8157-569">System.Management.Automation.ScriptBlock</span></span>

<span data-ttu-id="d8157-570">스크립트 블록의 명령을로 파이프 할 수 있습니다 `Invoke-Command` .</span><span class="sxs-lookup"><span data-stu-id="d8157-570">You can pipe a command in a script block to `Invoke-Command`.</span></span> <span data-ttu-id="d8157-571">`$Input`자동 변수를 사용 하 여 명령에서 입력 개체를 나타냅니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-571">Use the `$Input` automatic variable to represent the input objects in the command.</span></span>

## <span data-ttu-id="d8157-572">출력</span><span class="sxs-lookup"><span data-stu-id="d8157-572">OUTPUTS</span></span>

### <span data-ttu-id="d8157-573">Runspace (호출 된 명령의 출력 또는 호출 된 명령의 출력)를 실행 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-573">System.Management.Automation.PSRemotingJob, System.Management.Automation.Runspaces.PSSession, or the output of the invoked command</span></span>

<span data-ttu-id="d8157-574">**AsJob** 매개 변수를 사용 하는 경우이 cmdlet은 작업 개체를 반환 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-574">This cmdlet returns a job object, if you use the **AsJob** parameter.</span></span> <span data-ttu-id="d8157-575">**InDisconnectedSession** 매개 변수를 지정 하면는 `Invoke-Command` **PSSession** 개체를 반환 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-575">If you specify the **InDisconnectedSession** parameter, `Invoke-Command` returns a **PSSession** object.</span></span> <span data-ttu-id="d8157-576">그렇지 않으면 호출 된 명령의 출력을 반환 합니다 .이는 **ScriptBlock** 매개 변수의 값입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-576">Otherwise, it returns the output of the invoked command, which is the value of the **ScriptBlock** parameter.</span></span>

## <span data-ttu-id="d8157-577">참고</span><span class="sxs-lookup"><span data-stu-id="d8157-577">NOTES</span></span>

<span data-ttu-id="d8157-578">Windows Vista 이상 버전의 Windows 운영 체제에서 **ComputerName** 매개 변수를 사용 하 여 `Invoke-Command` 로컬 컴퓨터에서 명령을 실행 하려면 **관리자 권한으로 실행** 옵션을 사용 하 여 PowerShell을 실행 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-578">On Windows Vista, and later versions of the Windows operating system, to use the **ComputerName** parameter of `Invoke-Command` to run a command on the local computer, you must run PowerShell using the **Run as administrator** option.</span></span>

<span data-ttu-id="d8157-579">여러 컴퓨터에서 명령을 실행 하는 경우 PowerShell은 목록에 나타나는 순서 대로 컴퓨터에 연결 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-579">When you run commands on multiple computers, PowerShell connects to the computers in the order in which they appear in the list.</span></span> <span data-ttu-id="d8157-580">그러나 명령 출력은 원격 컴퓨터에서 수신 된 순서 대로 표시 됩니다 .이는 다를 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-580">However, the command output is displayed in the order that it's received from the remote computers, which might be different.</span></span>

<span data-ttu-id="d8157-581">실행 되는 명령으로 인해 발생 하는 오류는 `Invoke-Command` 명령 결과에 포함 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-581">Errors that result from the command that `Invoke-Command` runs are included in the command results.</span></span>
<span data-ttu-id="d8157-582">로컬 명령에서 종료 오류인 오류는 원격 명령에서 종료되지 않는 오류로 취급됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-582">Errors that would be terminating errors in a local command are treated as non-terminating errors in a remote command.</span></span> <span data-ttu-id="d8157-583">이 전략은 한 컴퓨터에서 종료 오류가 실행 되는 모든 컴퓨터에서 해당 명령을 닫지 않도록 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-583">This strategy makes sure that terminating errors on one computer don't close the command on all computers on which it's run.</span></span> <span data-ttu-id="d8157-584">이 방법은 원격 명령이 한 컴퓨터에서 실행되는 경우에도 사용됩니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-584">This practice is used even when a remote command is run on a single computer.</span></span>

<span data-ttu-id="d8157-585">원격 컴퓨터가 로컬 컴퓨터가 신뢰 하는 도메인에 없는 경우 컴퓨터에서 사용자 자격 증명을 인증 하지 못할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-585">If the remote computer isn't in a domain that the local computer trusts, the computer might not be able to authenticate the user's credentials.</span></span> <span data-ttu-id="d8157-586">WS-MANAGEMENT에서 신뢰할 수 있는 호스트 목록에 원격 컴퓨터를 추가 하려면 공급자에서 다음 명령을 사용 합니다 `WSMAN` `<Remote-Computer-Name>` . 여기서은 원격 컴퓨터의 이름입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-586">To add the remote computer to the list of trusted hosts in WS-Management, use the following command in the `WSMAN` provider, where `<Remote-Computer-Name>` is the name of the remote computer:</span></span>

`Set-Item -Path WSMan:\Localhost\Client\TrustedHosts -Value \<Remote-Computer-Name\>`

<span data-ttu-id="d8157-587">**InDisconnectedSession** 매개 변수를 사용 하 여 **PSSession** 의 연결을 끊으면 세션 상태가 **Disconnected** 이 고 availability가 **None** 입니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-587">When you disconnect a **PSSession** using the **InDisconnectedSession** parameter, the session state is **Disconnected** and the availability is **None**.</span></span> <span data-ttu-id="d8157-588">**State** 속성 값은 현재 세션을 기준으로 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-588">The value of the **State** property is relative to the current session.</span></span> <span data-ttu-id="d8157-589">**연결 끊김** 값은 **PSSession** 이 현재 세션에 연결 되지 않았음을 의미 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-589">A value of **Disconnected** means that the **PSSession** isn't connected to the current session.</span></span> <span data-ttu-id="d8157-590">그러나 모든 세션에서 **PSSession** 의 연결이 끊어졌음을 의미 하는 것은 아닙니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-590">However, it doesn't mean that the **PSSession** is disconnected from all sessions.</span></span> <span data-ttu-id="d8157-591">다른 세션에 연결되어 있을 수도 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-591">It might be connected to a different session.</span></span> <span data-ttu-id="d8157-592">세션에 연결하거나 다시 연결할 수 있는지 확인하려면 **Availability** 속성을 사용합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-592">To determine whether you can connect or reconnect to the session, use the **Availability** property.</span></span>

<span data-ttu-id="d8157-593">**Availability** 값이 **None** 이면 세션에 연결할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-593">An **Availability** value of **None** indicates that you can connect to the session.</span></span> <span data-ttu-id="d8157-594">**사용 중** 값은 다른 세션에 연결 되어 있으므로 **PSSession** 에 연결할 수 없음을 나타냅니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-594">A value of **Busy** indicates that you can't connect to the **PSSession** because it's connected to another session.</span></span> <span data-ttu-id="d8157-595">세션의 **State** 속성 값에 대 한 자세한 내용은 [RunspaceState](/dotnet/api/system.management.automation.runspaces.runspacestate)를 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="d8157-595">For more information about the values of the **State** property of sessions, see [RunspaceState](/dotnet/api/system.management.automation.runspaces.runspacestate).</span></span> <span data-ttu-id="d8157-596">세션의 **Availability** 속성 값에 대 한 자세한 내용은 [RunspaceAvailability](/dotnet/api/system.management.automation.runspaces.runspaceavailability)를 참조 하십시오.</span><span class="sxs-lookup"><span data-stu-id="d8157-596">For more information about the values of the **Availability** property of sessions, see [RunspaceAvailability](/dotnet/api/system.management.automation.runspaces.runspaceavailability).</span></span>

<span data-ttu-id="d8157-597">**HostName** 및 **SSHConnection** 매개 변수는 PowerShell 6.0부터 포함 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-597">The **HostName** and **SSHConnection** parameters were included starting with PowerShell 6.0.</span></span> <span data-ttu-id="d8157-598">Secure Shell (SSH)를 기반으로 PowerShell 원격 기능을 제공 하기 위해 추가 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-598">They were added to provide PowerShell remoting based on Secure Shell (SSH).</span></span> <span data-ttu-id="d8157-599">Powershell 및 SSH는 여러 플랫폼 (Windows, Linux, macOS)에서 지원 되며 powershell 원격은 PowerShell 및 SSH가 설치 되 고 구성 되는 이러한 플랫폼에 대해 작동 합니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-599">PowerShell and SSH are supported on multiple platforms (Windows, Linux, macOS) and PowerShell remoting works over these platforms where PowerShell and SSH are installed and configured.</span></span> <span data-ttu-id="d8157-600">이는 WinRM을 기반으로 하는 이전 Windows 전용 원격 시스템과 별개 이며, 많은 WinRM 특정 기능 및 제한 사항이 적용 되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-600">This is separate from the previous Windows only remoting that is based on WinRM and many of the WinRM specific features and limitations don't apply.</span></span> <span data-ttu-id="d8157-601">예를 들어 WinRM 기반 할당량, 세션 옵션, 사용자 지정 끝점 구성 및 연결 끊기/다시 연결 기능은 현재 지원 되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="d8157-601">For example WinRM based quotas, session options, custom endpoint configuration, and disconnect/reconnect features are currently not supported.</span></span> <span data-ttu-id="d8157-602">PowerShell SSH 원격을 설정 하는 방법에 대 한 자세한 내용은 [ssh를 통한 Powershell 원격](/powershell/scripting/learn/remoting/ssh-remoting-in-powershell-core)을 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="d8157-602">For more information about how to set up PowerShell SSH remoting, see [PowerShell Remoting Over SSH](/powershell/scripting/learn/remoting/ssh-remoting-in-powershell-core).</span></span>

## <span data-ttu-id="d8157-603">관련 링크</span><span class="sxs-lookup"><span data-stu-id="d8157-603">RELATED LINKS</span></span>

[<span data-ttu-id="d8157-604">about_PSSessions</span><span class="sxs-lookup"><span data-stu-id="d8157-604">about_PSSessions</span></span>](./About/about_PSSessions.md)

[<span data-ttu-id="d8157-605">about_Remote</span><span class="sxs-lookup"><span data-stu-id="d8157-605">about_Remote</span></span>](./About/about_Remote.md)

[<span data-ttu-id="d8157-606">about_Remote_Disconnected_Sessions</span><span class="sxs-lookup"><span data-stu-id="d8157-606">about_Remote_Disconnected_Sessions</span></span>](./About/about_Remote_Disconnected_Sessions.md)

[<span data-ttu-id="d8157-607">about_Remote_Troubleshooting</span><span class="sxs-lookup"><span data-stu-id="d8157-607">about_Remote_Troubleshooting</span></span>](./About/about_remote_troubleshooting.md)

[<span data-ttu-id="d8157-608">about_Remote_Variables</span><span class="sxs-lookup"><span data-stu-id="d8157-608">about_Remote_Variables</span></span>](./About/about_Remote_Variables.md)

[<span data-ttu-id="d8157-609">about_Scopes</span><span class="sxs-lookup"><span data-stu-id="d8157-609">about_Scopes</span></span>](./About/about_scopes.md)

[<span data-ttu-id="d8157-610">Enter-PSSession</span><span class="sxs-lookup"><span data-stu-id="d8157-610">Enter-PSSession</span></span>](Enter-PSSession.md)

[<span data-ttu-id="d8157-611">Exit-PSSession</span><span class="sxs-lookup"><span data-stu-id="d8157-611">Exit-PSSession</span></span>](Exit-PSSession.md)

[<span data-ttu-id="d8157-612">Get-PSSession</span><span class="sxs-lookup"><span data-stu-id="d8157-612">Get-PSSession</span></span>](Get-PSSession.md)

[<span data-ttu-id="d8157-613">Invoke-Item</span><span class="sxs-lookup"><span data-stu-id="d8157-613">Invoke-Item</span></span>](../Microsoft.PowerShell.Management/Invoke-Item.md)

[<span data-ttu-id="d8157-614">New-PSSession</span><span class="sxs-lookup"><span data-stu-id="d8157-614">New-PSSession</span></span>](New-PSSession.md)

[<span data-ttu-id="d8157-615">Remove-PSSession</span><span class="sxs-lookup"><span data-stu-id="d8157-615">Remove-PSSession</span></span>](Remove-PSSession.md)

[<span data-ttu-id="d8157-616">WSMan 공급자</span><span class="sxs-lookup"><span data-stu-id="d8157-616">WSMan Provider</span></span>](../Microsoft.WsMan.Management/About/about_WSMan_Provider.md)
