...
함수 및 스크립트 블록처럼, `Configuration` 내에서 모든 PowerShell 언어를 사용할 수 있습니다. 사용하는 문은 `Configuration`을 호출하여 `.mof` 파일을 컴파일할 때만 평가됩니다. 아래 예제에서는 개념을 설명하는 시나리오를 보여 줍니다. 조건문 및 루프는 매개 변수 및 구성 데이터에서 더 자주 사용됩니다.
