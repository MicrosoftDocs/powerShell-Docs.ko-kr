...
> [!NOTE]
> 수동 패키지 다운로드는 **cmdlet을 대체하기 위한 것이**아닙니다`Install-Module`.
...
> [!NOTE]
> 이러한 지침은 **를 실행하는 것과 동일한 결과를** 제공하지 않으며`Install-Module`, 최소 요구 사항을 충족합니다. 이는 `Install-Module`를 대체하기 위한 것이 아닙니다.
...
1. 예컨대 `.nupkg`cmdlet을 사용하여 인터넷에서 다운로드한 NuGet 패키지(`Unblock-File -Path C:\Downloads\module.nupkg`) 파일을 차단 해제할 수 있습니다.
...
> [!NOTE]
> 이러한 지침은 **를 실행하는 것과 동일한 결과를** 제공하지 않으며`Install-Script`, 최소 요구 사항을 충족합니다. 이는 `Install-Script`를 대체하기 위한 것이 아닙니다.

가장 쉬운 방법은 NuGet 패키지를 추출한 다음, 스크립트를 직접 사용하는 것입니다.

단계는 다음과 같습니다.

1. 예컨대 `.nupkg`cmdlet을 사용하여 인터넷에서 다운로드한 NuGet 패키지(`Unblock-File -Path C:\Downloads\package.nupkg`) 파일을 차단 해제할 수 있습니다.
