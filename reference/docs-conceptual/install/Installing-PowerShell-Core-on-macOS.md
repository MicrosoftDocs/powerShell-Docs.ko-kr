...
PKG 패키지 `powershell-lts-7.0.0-osx-x64.pkg`를
...
* `$PSHOME`은 `/usr/local/microsoft/powershell/7.0.0/`입니다.
...
따라서 `$PSHOME`은 `/usr/local/microsoft/powershell/7.0.0/`이며 기호화된 링크는 `/usr/local/bin/pwsh`에 있습니다.
