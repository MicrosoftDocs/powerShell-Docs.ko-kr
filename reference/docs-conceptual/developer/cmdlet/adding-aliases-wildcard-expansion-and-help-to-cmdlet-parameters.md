...
Cmdlet은 시스템 수정과 사용자 피드백을 지 원하는 매개 변수를 정의 해야 합니다. Cmdlet은 특정 식별자로 시스템을 수정할 수 있도록 `Name` 매개 변수 또는 그에 해당 하는 값을 정의 해야 합니다. 또한 cmdlet은 `Force` 및 `PassThru` 매개 변수를 정의 해야 합니다. 이러한 매개 변수에 대 한 자세한 내용은 [시스템을 수정 하는 Cmdlet 만들기](./creating-a-cmdlet-that-modifies-the-system.md)를 참조 하세요.
...
[Aliasattribute](/dotnet/api/System.Management.Automation.AliasAttribute) 특성을 사용 하는 것 외에도 Windows PowerShell 런타임은 별칭이 지정 되지 않은 경우에도 부분 이름 일치를 수행 합니다. 예를 들어 cmdlet에 `FileName` 매개 변수가 있고 `F`로 시작 하는 유일한 매개 변수인 경우에는 사용자가 `Filename`, `Filenam`, `File`, `F`을 입력할 수 있습니다.

## <a name="creating-help-for-parameters"></a>매개 변수에 대 한 도움말 만들기

Windows PowerShell을 사용 하 여 cmdlet 매개 변수에 대 한 도움말을 만들 수 있습니다. 시스템 수정 및 사용자 의견에 사용 되는 매개 변수에 대해이 작업을 수행 합니다. 도움말을 지 원하는 각 매개 변수에 대해 [system.object](/dotnet/api/System.Management.Automation.ParameterAttribute) 특성 선언에서 `HelpMessage` attribute 키워드를 설정할 수 있습니다. 이 키워드는 매개 변수를 사용할 때 사용자에 게 표시 되는 텍스트를 정의 합니다. @No__t-0 키워드를 설정 하 여 메시지에 사용할 리소스의 기본 이름을 식별할 수도 있습니다. 이 키워드를 설정 하는 경우에는 `HelpMessageResourceId` 키워드를 설정 하 여 리소스 식별자를 지정 해야 합니다.
...
- Windows PowerShell을 시작 하 고 ProcessName 매개 변수에 대 @no__t 한 별칭을 사용 하 여 프로세스를 중지 하려면 Stop Proc를 사용 합니다.
...
- 이제 와일드 카드 패턴과 일치 하는 모든 프로세스를 중지 하는 다음과 같은 항목을 만듭니다. "* note @ no__t-0". 패턴과 일치 하는 각 프로세스를 중지 하기 전에 메시지가 표시 됩니다.
