...
1. Windows 탐색기를 열고 Samples 디렉터리 아래의 GetProcessSample05 디렉터리로 이동 합니다.

   Windows PowerShell 2.0 SDK가 설치 된 상태에서 GetProcessSample05 폴더로 이동 합니다. 기본 위치는 C:\Program Files (x86) \Microsoft SDKs\Windows\v7.0\Samples\sysmgmt\WindowsPowerShell\csharp\GetProcessSample05.
...
- 해당 매개 변수를 지정 하면 파이프라인에서 입력을 사용할 수 있습니다. 입력은 개체 또는 속성 이름이 매개 변수 이름과 동일한 개체의 속성에서 가져올 수 있습니다.
...
이 샘플에서는 지정 된 프로세스 목록을 표시 하는 cmdlet을 만드는 방법을 보여 줍니다.
