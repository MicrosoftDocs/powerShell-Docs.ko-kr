...
공용 리소스 스키마를 사용 하는 MOF 파일을 만들어 관리 OData 웹 서비스에서 노출 하는 리소스를 정의 합니다. 각 리소스는 파일에서 클래스로 정의 되 고 속성은 클래스 멤버로 정의 됩니다. MOF 파일에 사용 되는 스키마에 대 한 자세한 내용은 [공용 리소스 스키마](./public-resource-schema.md)를 참조 하세요.

## <a name="example-mof-file"></a>MOF 파일 예
