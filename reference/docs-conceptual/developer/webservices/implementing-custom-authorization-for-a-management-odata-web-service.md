...
Windows PowerShell 웹 서비스를 사용 하려면 Windows PowerShell cmdlet을 노출 하기 위해 [CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) 인터페이스를 구현 하는 타사가 필요 합니다. 이 인터페이스는 웹 서비스에 대 한 사용자 권한 부여를 수행 합니다. 인터페이스를 구현 하는 코드를 작성 한 후에는 웹 응용 프로그램에서 사용할 DLL로 컴파일해야 합니다.
...
다음 예에서는 XML 파일에서 권한 부여 정책을 구문 분석 하는 클래스를 만듭니다.
