...
`Format-Custom` 는 단순히 테이블이 나 목록이 아닌 보기를 표시 하도록 설계 되었습니다. PowerShell 디렉터리의 파일에 정의 된 뷰를 사용 `*format.PS1XML` 하거나 새 types.ps1xml 파일에서 고유한 뷰를 만들고 cmdlet을 사용 `Update-FormatData` 하 여 powershell에 추가할 수 있습니다.
