...
사용자 지정 이벤트를 구독하지 않는 경우 이벤트를 삭제하려면 프로그램 상태를 변경하거나 Windows PowerShell 세션을 닫아야 합니다.
...
이 명령은 Windows PowerShell 이벤트 큐에 새 이벤트를 만들고
...
**새 이벤트** 에서 발생 하는 이벤트는 Windows PowerShellevent 큐에 자동으로 추가 되므로 해당 이벤트를 등록할 필요가 없습니다.
