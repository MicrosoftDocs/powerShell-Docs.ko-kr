...
기존 파일의 끝에 출력을 추가 합니다. **인코딩을** 지정 하지 않으면 cmdlet은 기본 인코딩을 사용 합니다. 해당 인코딩은 대상 파일의 인코딩과 일치 하지 않을 수 있습니다. 이 동작은 리디렉션 연산자 ()와 동일 합니다 `>>` .
...
대상 파일의 인코딩 유형을 지정합니다. 기본값은 `unicode`입니다.
...
입력 개체는 터미널에 있을 때 자동으로 서식이 지정 되지만 cmdlet을 사용 `Format-*` 하 여 파일에 대 한 출력 형식을 명시적으로 제어할 수 있습니다. 예를 들어 `Get-Date | Format-List | Out-File out.txt`
