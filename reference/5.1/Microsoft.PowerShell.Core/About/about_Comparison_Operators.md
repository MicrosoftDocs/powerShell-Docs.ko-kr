...
|             |              |                                             |
| 유형        | -is          | 두 개체가 같을 경우 true를 반환 합니다.    |
|             |              | type                                        |
...
> [!NOTE]
> 값을 비교 해야 하는 경우 `$null` `$null` 비교의 왼쪽에를 넣어야 합니다. `$null` **개체 []** 와 비교할 때 비교 개체가 배열 이기 때문에 결과가 **False** 입니다. 배열을와 비교할 때 `$null` 비교는 `$null` 배열에 저장 된 모든 값을 필터링 합니다. 다음은 그 예입니다. 
>
> ```powershell
...
`-replace`연산자는 정규식을 사용 하 여 값의 일부 또는 전체를 지정 된 값으로 바꿉니다. `-replace`파일 이름 바꾸기와 같은 많은 관리 작업에 대해 연산자를 사용할 수 있습니다. 예를 들어 다음 명령은 모든 .txt 파일의 파일 이름 확장명을 .log로 변경 합니다.

```powershell
Get-ChildItem *.txt | Rename-Item -NewName { $_.name -replace '\.txt$','.log' }
```

연산자의 구문은 `-replace` 다음과 같습니다 `<original>` . 여기서 자리 표시자는 바꿀 문자를 나타내고 `<substitute>` 자리 표시자는 바꿀 문자를 나타냅니다.
...
또한 정규식을 사용 하 여 캡처링 그룹 및 대체를 사용 하 여 텍스트를 동적으로 바꿀 수 있습니다. 자세한 내용은 [about_Regular_Expressions](about_Regular_Expressions.md)를 참조 하세요.

#### <a name="substitutions-in-regular-expressions"></a>정규식의 대체

또한 캡처링 그룹은 문자열에서 참조할 수 있습니다 \<substitute\> .
그룹 식별자 앞에 문자를 사용 하 여이 작업을 수행 `$` 합니다.

캡처링 그룹을 참조 하는 두 가지 방법 중에는 **숫자** 와 **이름을** 기준으로 합니다.

- **Number** -
   캡처링 그룹은 왼쪽에서 오른쪽으로 번호가 매겨집니다.
...
> [!WARNING]
> 문자는 `$` 문자열 확장에 사용 되므로 대체를 포함 하는 리터럴 문자열을 사용 하거나 문자를 이스케이프 해야 `$` 합니다.
>
> ```powershell
...
> ```
>
> 또한 `$` 문자가 대체에 사용 되므로 문자열의 모든 인스턴스를 이스케이프 처리 해야 합니다.
>
> ```powershell
> '5.72' -replace '(.+)', '$$$1'
> ```
