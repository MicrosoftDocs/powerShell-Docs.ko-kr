...
이 예에서는 `$DebugPreference` 변수의 기본값 **SilentlyContinue** 의 효과를 보여 줍니다. 기본적으로 `Write-Debug` cmdlet의 디버그 메시지가 표시 되지 않고 처리가 계속 됩니다. **디버그** 매개 변수를 사용 하는 경우 단일 명령에 대 한 기본 설정을 재정의 합니다. 사용자에 게 확인 메시지를 표시 합니다.
...
- **Suspend** : `Write-Information` 계속 하기 전에 사용자가 메시지를 볼 수 있도록 명령을 수행한 후 워크플로 작업을 자동으로 일시 중단 합니다. 사용자의 판단에 의해 워크플로를 다시 시작할 수 있습니다.
...
PowerShell 세션에서 허용 되는 별칭 수를 결정 합니다. 기본값은 **4096** 이며 대부분의 용도에 충분 해야 합니다. 요구 사항에 맞게 조정할 수 있습니다 `$MaximumAliasCount` .

**유효한 값** : 1024-32768 ( `Int32` )

**기본값** : 4096

시스템에서 별칭을 계산 하려면 다음을 입력 합니다.

```powershell
(Get-Alias).count
```

### <a name="maximumdrivecount"></a>\$Maximum드라이브 Ecount

지정 된 세션에서 허용 되는 PowerShell 드라이브 수를 결정 합니다. 예를 들어 PowerShell 공급자에 의해 노출 되 고 드라이브 (예: 및 드라이브)로 표시 되는 파일 시스템 드라이브 및 데이터 저장소입니다 `Alias:` `HKLM:` .
...
세션에 대 한 오류 기록에 저장 되는 오류 수를 결정 합니다.

**유효한 값** : 256-32768 ( `Int32` )

**기본값** : 256

유지 되는 각 오류를 나타내는 개체는 `$Error` 자동 변수에 저장 됩니다. `$Error` 오류 레코드 개체의 배열을 포함 합니다. 가장 최근의 오류는 배열의 첫 번째 개체 `$Error[0]` 입니다.

시스템에서 발생 하는 오류 수를 계산 하려면 `$Error` 배열의 **count** 속성을 사용 합니다.

```powershell
$Error.count
```

특정 오류를 표시 하려면 `[0]` 배열 표기법을 사용 하 여 가장 최근의 오류를 확인 합니다.

```powershell
$Error[0]
```

보관 된 가장 오래 된 오류를 표시 하려면 다음을 입력 합니다.

```powershell
$Error[($Error.Count -1]
```

**Force** 매개 변수는 **ErrorRecord** 개체의 특수 한 서식을 재정의 하 고 기존 형식으로 되돌립니다. **ErrorRecord** 개체의 속성을 표시 하려면 다음 명령을 입력 합니다.

```powershell
$Error[0] | Format-List -Property * -Force
```

이 예에서는 `$Error.Count` 오류 수를 표시 합니다. 오류 기록에서 모든 오류를 삭제 하려면 `Clear` 오류 배열의 메서드를 사용 합니다.
...
오류 배열의 모든 속성 및 메서드를 찾으려면 `Get-Member` 해당 **InputObject** 매개 변수와 함께 cmdlet을 사용 합니다. **InputObject** 매개 변수를 사용 하는 경우 `Get-Member` 컬렉션의 속성 및 메서드를 표시 합니다.

```powershell
Get-Member -InputObject $Error
```

개체의 컬렉션을로 파이프 하면에서 `Get-Member` `Get-Member` 컬렉션에 있는 개체의 속성 및 메서드를 표시 합니다.
...
지정 된 세션에서 허용 되는 함수 수를 결정 합니다.

**유효한 값** : 1024-32768 ( `Int32` )

**기본값** : 4096

세션에서 함수를 보려면 `Function:` powershell 공급자가 제공 하는 powershell 드라이브를 사용 `Function` 합니다. 공급자에 대 한 자세한 내용은 `Function` 을 [about_Function_Provider](about_Function_Provider.md)하십시오.

현재 세션의 함수를 나열 하려면 다음을 입력 합니다.

```powershell
Get-ChildItem Function:
```

현재 세션의 함수 수를 계산 하려면 다음을 입력 합니다.
...
자동 변수, 기본 설정 변수 및 명령 및 스크립트에서 만든 변수를 포함 하 여 지정 된 세션에서 허용 되는 변수 수를 결정 합니다.

**유효한 값** : 1024-32768 ( `Int32` )

**기본값** : 4096

세션의 변수를 보려면 `Get-Variable` powershell `Variable:` 드라이브 및 powershell 공급자의 cmdlet 및 기능을 사용 합니다 `Variable` . 자세한 내용은 [about_Variable_Provider](about_Variable_Provider.md)를 참조 하세요.

시스템에서 현재 변수 수를 찾으려면 다음을 입력 합니다.
...
**기본값** : ASCIIEncoding 개체 (ASCIIEncoding)
