...
전달 된 컬렉션이 단일 요소로 처리 되는 **ValueFromRemainingArguments** 와 함께 컬렉션을 사용 하는 경우 알려진 문제가 있습니다.

다음 예제에서는이 알려진 문제를 보여 줍니다. **나머지** 매개 변수는 인덱스 **0** 에서 인덱스 **1** 에 **있는** 매개 변수 **를 포함 해야** 합니다.
대신 두 요소가 단일 엔터티로 결합 됩니다.
...
**ValidateRange** 특성은 각 매개 변수 또는 변수 값에 대 한 숫자 범위를 지정 합니다. 값이 범위를 벗어나면 PowerShell에서 오류를 생성 합니다. 다음 예에서는 **시도** 매개 변수의 값이 0에서 10 사이 여야 합니다.
