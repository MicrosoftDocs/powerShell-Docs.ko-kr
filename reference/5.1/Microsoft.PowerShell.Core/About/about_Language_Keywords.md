...
File 매개 변수와 함께를 사용 하는 경우 스크립트를 `powershell.exe` **File** `.ps1` 실행 하는 동안 발생 하는 오류 또는 예외를 처리 하기 위한 지침을 스크립트 파일 자체에 포함 해야 합니다. `exit`문을 사용 하 여 스크립트의 실행 후 상태를 나타내야 합니다.

과 사이의 숫자가 `[int]::MinValue` 모두 `[int]::MaxValue` 허용 됩니다.
