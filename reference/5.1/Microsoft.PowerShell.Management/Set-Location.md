...
**Stackname** 매개 변수를 사용 하 여 명명 된 위치 스택을 현재 위치 스택으로 만들 수도 있습니다. 위치 스택에 대한 자세한 내용은 참고를 참조하세요.
...
작은따옴표는 Windows PowerShell이 어떤 문자도 이스케이프 시퀀스로 해석하지 않도록 지시합니다.
...
`*-Location` **Stackname** 매개 변수를 사용 하 여 다른 스택을 지정 하지 않는 한 cmdlet은 현재 스택에서 작동 합니다.
