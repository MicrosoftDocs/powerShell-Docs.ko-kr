...
PowerShell 3.0부터 **메시지** 매개 변수를 사용 하 여 대화 상자에서 사용자 이름과 암호를 입력 하 라는 사용자 지정 메시지를 지정할 수 있습니다.

Cmdlet은 사용자에 게 `Get-Credential` 암호나 사용자 이름 및 암호를 묻는 메시지를 표시 합니다. 기본적으로 사용자에게 입력을 요구하는 인증 대화 상자가 나타납니다. 그러나 PowerShell 콘솔과 같은 일부 호스트 프로그램에서는 레지스트리 항목을 변경 하 여 명령줄에서 사용자에 게 메시지를 표시할 수 있습니다. 이 레지스트리 항목에 대한 자세한 내용은 설명 및 예제를 참조하세요.
...
명령을 입력하면 사용자 이름 및 암호를 요청하는 대화 상자가 나타납니다. 요청 된 정보를 입력 하면 cmdlet이 사용자의 자격 증명을 나타내는 **PSCredential** 개체를 만들어 변수에 저장 합니다 `$c` .
...
**PromptForCredential** 메서드를 사용 하는 대신 cmdlet을 사용할 수 `Get-Credential` 있습니다. **PromptForCredential** 을 사용할 경우 메시지 상자에 표시되는 설명, 메시지 및 사용자 이름을 지정할 수 있습니다.
...
이 예제에서는 대화 상자를 사용하지 않고 명령줄에서 표시되도록 레지스트리를 수정하는 방법을 보여 줍니다.

이 명령은 **ConsolePrompting** 레지스트리 항목을 만들어  해당 값을 True로 설정합니다. 이 명령을 실행 하려면 "관리자 권한으로 실행" 옵션을 사용 하 여 PowerShell을 시작 합니다.

메시지를 표시 하는 대화 상자를 사용 하려면 ConsolePrompting의 값을 false ($false)로 설정 하거나 cmdlet을 사용 하 여 `Remove-ItemProperty` 삭제 합니다.

ConsolePrompting 레지스트리 항목은 PowerShell 콘솔과 같은 일부 호스트 프로그램에서 작동 합니다. 일부 호스트 프로그램에서는 작동하지 않을 수 있습니다.
...
인증 프롬프트가 대화 상자에 나타나면 사용자가 지정된 사용자 이름을 편집할 수 있습니다.
하지만 프롬프트가 명령줄에 표시될 때에는 사용자 이름을 변경할 수 없습니다. 공유 함수 또는 스크립트에서 이 매개 변수를 사용할 때 가능한 모든 표시 방법을 고려해보세요.
...
기본적으로 인증 프롬프트가 대화 상자에 표시됩니다. 명령줄에 인증 프롬프트를 표시 하려면 **ConsolePrompting** 레지스트리 항목 ()을 추가 하 `HKLM:\SOFTWARE\Microsoft\PowerShell\1\ShellIds\ConsolePrompting` 고 해당 값을 **True** 로 설정 합니다.
**ConsolePrompting** 레지스트리 항목이 존재하지 않거나 해당 값이 False이면 인증 프롬프트가 대화 상자에 나타납니다. 지침에 대해서는 예제를 참조하세요.

**ConsolePrompting** 레지스트리 항목은 PowerShell 콘솔에서 작동 하지만 일부 호스트 프로그램에서는 작동 하지 않습니다.

예를 들어 PowerShell ISE (통합 스크립팅 환경)에는 영향을 주지 않습니다. **ConsolePrompting** 레지스트리 항목의 효과에 대해서는 호스트 프로그램에 대한 도움말 항목을 참조하세요.
